...
const Rb12Form = require('../../models/RB/Rb12Form'); // Ensure correct model path

// ✅ POST route for RB1.2 form submission
router.post('/', async (req, res) => {
    const { teacherName, teacherDepartment, activityType, activityLevel, linkEvidence, formId, score } = req.body;

    if (!teacherName || !teacherDepartment || !activityType || !activityLevel || !linkEvidence || !formId) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    try {
        const newForm = new Rb12Form({
            teacherName,
            teacherDepartment,
            activityType,
            activityLevel,
        ...
        await newForm.save();
        res.status(201).json({ message: 'RB1.2 Form submitted successfully!', newForm });
    } catch (error) {
        console.error('Error submitting RB1.2 form:', error.message);
        res.status(500).json({ message: 'Internal server error', error: error.message });
    }
});

// ✅ GET route to fetch all RB1.2 submissions
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Rb12Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
        return res.status(200).json(submissions);
    } catch (error) {
        console.error('Error fetching RB1.2 submissions:', error);
...
router.get('/check-submission', async (req, res) => {
    ...
    try {
        const existingSubmission = await Rb12Form.findOne({ teacherName, teacherDepartment });
...
// ✅ PUT route to update an existing RB1.2 submission
router.put('/:id', async (req, res) => {
    const { activityType, activityLevel, linkEvidence, score } = req.body;

    if (!activityType || !activityLevel || !linkEvidence) {
        return res.status(400).json({ message: 'All fields are required to update the form' });
    }

    try {
        const updatedForm = await Rb12Form.findByIdAndUpdate(
            req.params.id,
            { activityType, activityLevel, linkEvidence, score },
            { new: true }
        );
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Rb12Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Rb12Form.findByIdAndDelete(req.params.id);
