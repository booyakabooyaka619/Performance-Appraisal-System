...
const Rb9Form = require('../../models/RB/Rb9Form'); // ✅ Ensure correct model path

// ✅ POST route for RB9 form submission
router.post('/', async (req, res) => {
    ...
    try {
        ...
        await newForm.save();
        res.status(201).json({ message: 'RB9 Form submitted successfully!', newForm });
    } catch (error) {
        console.error('Error submitting RB9 form:', error.message);
        res.status(500).json({ message: 'Internal server error', error: error.message });
    }
});

// ✅ GET route to fetch all RB9 submissions
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Rb9Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
        return res.status(200).json(submissions);
    } catch (error) {
        console.error('Error fetching RB9 submissions:', error);
...
router.get('/check-submission', async (req, res) => {
    ...
    try {
        const existingSubmission = await Rb9Form.findOne({ teacherName, teacherDepartment });
...
// ✅ PUT route to update an existing RB9 submission
router.put('/:id', async (req, res) => {
    ...
    try {
        const updatedForm = await Rb9Form.findByIdAndUpdate(
            req.params.id,
            {
                technicalContent,
                presentationSkills,
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Rb9Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Rb9Form.findByIdAndDelete(req.params.id);
...
router.put('/toggle-edit-review/:id', async (req, res) => {
    try {
        ...
        res.status(200).json({
            message: `Review edit mode ${form.isReviewEditable ? 'enabled' : 'disabled'}`,
            form
        });
    } catch (error) {
