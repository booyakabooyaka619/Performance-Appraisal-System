...
const Rb6Form = require('../../models/RB/Rb6Form'); // Adjust path as needed

// ✅ POST route for RB6 form submission
router.post('/', async (req, res) => {
    const { teacherName, teacherDepartment, awardType, linkEvidence, formId, score } = req.body;

    if (!teacherName || !teacherDepartment || !awardType || !linkEvidence || !formId) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    try {
        const newForm = new Rb6Form({
            teacherName,
            teacherDepartment,
            awardType,
            linkEvidence,
        ...
        await newForm.save();
        res.status(201).json({ message: 'RB6 Form submitted successfully!', newForm });
    } catch (error) {
        console.error('Error submitting RB6 form:', error.message);
        res.status(500).json({ message: 'Internal server error', error: error.message });
    }
});

// ✅ GET route to fetch all RB6 submissions
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Rb6Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
        return res.status(200).json(submissions);
    } catch (error) {
        console.error('Error fetching RB6 submissions:', error);
...
router.get('/check-submission', async (req, res) => {
    ...
    try {
        const existingSubmission = await Rb6Form.findOne({ teacherName, teacherDepartment });
...
// ✅ PUT route to update an existing RB6 submission
router.put('/:id', async (req, res) => {
    const { awardType, linkEvidence, score } = req.body;

    if (!awardType || !linkEvidence) {
        return res.status(400).json({ message: 'All fields are required to update the form' });
    }

    try {
        const updatedForm = await Rb6Form.findByIdAndUpdate(
            req.params.id,
            { awardType, linkEvidence, score },
            { new: true }
        );
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Rb6Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Rb6Form.findByIdAndDelete(req.params.id);
