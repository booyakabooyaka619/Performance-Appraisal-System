...
const Ab6Form = require('../../models/AB/Ab6Form'); // ✅ Ensure correct model path

// ✅ POST route for AB6 form submission
router.post('/', async (req, res) => {
    const { teacherName, teacherDepartment, designation, certification, execution, linkEvidence, formId, score } = req.body;

    if (!teacherName || !teacherDepartment || !designation || !certification || !execution || !linkEvidence || !formId) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    try {
        const newForm = new Ab6Form({
            teacherName,
            teacherDepartment,
            designation,
            certification,
        ...
        await newForm.save();
        res.status(201).json({ message: 'AB6 Form submitted successfully!', newForm });
    } catch (error) {
        console.error('Error submitting AB6 form:', error.message);
        res.status(500).json({ message: 'Internal server error', error: error.message });
    }
});

// ✅ GET route to fetch all AB6 submissions
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Ab6Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
        return res.status(200).json(submissions);
    } catch (error) {
        console.error('Error fetching AB6 submissions:', error);
...
router.get('/check-submission', async (req, res) => {
    ...
    try {
        const existingSubmission = await Ab6Form.findOne({ teacherName, teacherDepartment });
...
// ✅ PUT route to update an existing AB6 submission
router.put('/:id', async (req, res) => {
    const { designation, certification, execution, linkEvidence, score } = req.body;

    if (!designation || !certification || !execution || !linkEvidence) {
        return res.status(400).json({ message: 'All fields are required to update the form' });
    }

    try {
        const updatedForm = await Ab6Form.findByIdAndUpdate(
            req.params.id,
            { designation, certification, execution, linkEvidence, score },
            { new: true }
        );
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Ab6Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Ab6Form.findByIdAndDelete(req.params.id);
