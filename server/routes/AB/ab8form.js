...
const Ab8Form = require('../../models/AB/Ab8Form'); // Ensure correct model path

// ✅ POST route for AB8 form submission
router.post('/', async (req, res) => {
    const { teacherName, teacherDepartment, typeOfMembership, positionHeld, linkEvidence, formId, score } = req.body;

    if (!teacherName || !teacherDepartment || !typeOfMembership || !positionHeld || !linkEvidence || !formId) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    try {
        const newForm = new Ab8Form({
            teacherName,
            teacherDepartment,
            typeOfMembership,
            positionHeld,
        ...
        await newForm.save();
        res.status(201).json({ message: 'AB8 Form submitted successfully!', newForm });
    } catch (error) {
        console.error('Error submitting AB8 form:', error.message);
        res.status(500).json({ message: 'Internal server error', error: error.message });
    }
});

// ✅ GET route to fetch all AB8 submissions
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Ab8Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
        return res.status(200).json(submissions);
    } catch (error) {
        console.error('Error fetching AB8 submissions:', error);
...
router.get('/check-submission', async (req, res) => {
    ...
    try {
        const existingSubmission = await Ab8Form.findOne({ teacherName, teacherDepartment });
...
// ✅ PUT route to update an existing AB8 submission
router.put('/:id', async (req, res) => {
    const { typeOfMembership, positionHeld, linkEvidence, score } = req.body;

    if (!typeOfMembership || !positionHeld || !linkEvidence) {
        return res.status(400).json({ message: 'All fields are required to update the form' });
    }

    try {
        const updatedForm = await Ab8Form.findByIdAndUpdate(
            req.params.id,
            { typeOfMembership, positionHeld, linkEvidence, score },
            { new: true }
        );
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Ab8Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Ab8Form.findByIdAndDelete(req.params.id);
