...
const Ab5Form = require('../../models/AB/Ab5Form'); // ✅ Ensure correct model path

// ✅ POST route for AB5 form submission
router.post('/', async (req, res) => {
    const { teacherName, teacherDepartment, roleType, noOfRoles, linkEvidence, formId, score } = req.body;

    if (!teacherName || !teacherDepartment || !roleType || !noOfRoles || !linkEvidence || !formId) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    try {
        const newForm = new Ab5Form({
            teacherName,
            teacherDepartment,
            roleType,
            noOfRoles,
        ...
        await newForm.save();
        res.status(201).json({ message: 'AB5 Form submitted successfully!', newForm });
    } catch (error) {
        console.error('Error submitting AB5 form:', error.message);
        res.status(500).json({ message: 'Internal server error', error: error.message });
    }
});

// ✅ GET route to fetch all AB5 submissions
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Ab5Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
        return res.status(200).json(submissions);
    } catch (error) {
        console.error('Error fetching AB5 submissions:', error);
...
router.get('/check-submission', async (req, res) => {
    ...
    try {
        const existingSubmission = await Ab5Form.findOne({ teacherName, teacherDepartment });
...
// ✅ PUT route to update an existing AB5 submission
router.put('/:id', async (req, res) => {
    const { roleType, noOfRoles, linkEvidence, score } = req.body;

    if (!roleType || !noOfRoles || !linkEvidence) {
        return res.status(400).json({ message: 'All fields are required to update the form' });
    }

    try {
        const updatedForm = await Ab5Form.findByIdAndUpdate(
            req.params.id,
            { roleType, noOfRoles, linkEvidence, score },
            { new: true }
        );
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Ab5Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Ab5Form.findByIdAndDelete(req.params.id);
