...
const Ab4Form = require('../../models/AB/Ab4Form'); // Adjust the model path as needed

// ✅ POST route for AB4 form submission
router.post('/', async (req, res) => {
    ...
    if (
        !teacherName || !teacherDepartment || !speakerCategory || !eventLevel ||
        !position || !duration || !participation || !linkEvidence || !formId
    ) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    try {
        const newForm = new Ab4Form({
            teacherName,
            teacherDepartment,
            speakerCategory,
            eventLevel,
        ...
        await newForm.save();
        res.status(201).json({ message: 'AB4 Form submitted successfully!', newForm });
    } catch (error) {
        console.error('Error submitting AB4 form:', error.message);
        res.status(500).json({ message: 'Internal server error', error: error.message });
    }
});

// ✅ GET route to fetch all AB4 submissions
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Ab4Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
        return res.status(200).json(submissions);
    } catch (error) {
        console.error('Error fetching AB4 submissions:', error);
...
router.get('/check-submission', async (req, res) => {
    ...
    try {
        const existingSubmission = await Ab4Form.findOne({ teacherName, teacherDepartment });
...
// ✅ PUT route to update an existing AB4 submission
router.put('/:id', async (req, res) => {
    ...
    if (!speakerCategory || !eventLevel || !position || !duration || !participation || !linkEvidence) {
        return res.status(400).json({ message: 'All fields are required to update the form' });
    }

    try {
        const updatedForm = await Ab4Form.findByIdAndUpdate(
            req.params.id,
            {
                speakerCategory,
                eventLevel,
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Ab4Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Ab4Form.findByIdAndDelete(req.params.id);
