...
const Ab2Form = require('../../models/AB/Ab2Form'); // Make sure the path is correct

// ✅ POST route for AB2 form submission
router.post('/', async (req, res) => {
    const { teacherName, teacherDepartment, registeredStudents, activityDuration, studentFeedback, linkEvidence, formId, score } = req.body;

    if (!teacherName || !teacherDepartment || !registeredStudents || !activityDuration || !studentFeedback || !linkEvidence || !formId) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    try {
        const newForm = new Ab2Form({
            teacherName,
            teacherDepartment,
            registeredStudents,
            activityDuration,
        ...
        await newForm.save();
        res.status(201).json({ message: 'AB2 Form submitted successfully!', newForm });
    } catch (error) {
        console.error('Error submitting AB2 form:', error.message);
        res.status(500).json({ message: 'Internal server error', error: error.message });
    }
});

// ✅ GET route to fetch all AB2 submissions
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Ab2Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
        return res.status(200).json(submissions);
    } catch (error) {
        console.error('Error fetching AB2 submissions:', error);
...
router.get('/check-submission', async (req, res) => {
    ...
    try {
        const existingSubmission = await Ab2Form.findOne({ teacherName, teacherDepartment });
...
// ✅ PUT route to update an existing AB2 submission
router.put('/:id', async (req, res) => {
    const { registeredStudents, activityDuration, studentFeedback, linkEvidence, score } = req.body;
    ...
    try {
        const updatedForm = await Ab2Form.findByIdAndUpdate(
            req.params.id,
            { registeredStudents, activityDuration, studentFeedback, linkEvidence, score },
            { new: true }
        );
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Ab2Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Ab2Form.findByIdAndDelete(req.params.id);
...
// ✅ Submit RO review and lock editing
