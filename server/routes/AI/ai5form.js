...
const Ai5Form = require('../../models/AI/Ai5Form'); // Ensure correct model path

// ✅ POST route for AI3.4 form submission
router.post('/', async (req, res) => {
    const { teacherName, teacherDepartment, courseLabName, attainmentLevel, linkEvidence, formId, score } = req.body;

    if (!teacherName || !teacherDepartment || !courseLabName || !attainmentLevel || !linkEvidence || !formId) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    try {
        const newForm = new Ai5Form({
            teacherName,
            teacherDepartment,
            courseLabName,
            attainmentLevel,
...
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Ai5Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
        return res.status(200).json(submissions);
    } catch (error) {
        console.error('Error fetching AI3.4 submissions:', error);
...
router.get('/check-submission', async (req, res) => {
    ...
    try {
        const existingSubmission = await Ai5Form.findOne({ teacherName, teacherDepartment });
...
router.put('/:id', async (req, res) => {
    const { courseLabName, attainmentLevel, linkEvidence, score } = req.body;

    if (!courseLabName || !attainmentLevel || !linkEvidence) {
        return res.status(400).json({ message: 'All fields are required to update the form' });
    }

    try {
        const updatedForm = await Ai5Form.findByIdAndUpdate(
            req.params.id,
            { courseLabName, attainmentLevel, linkEvidence, score },
            { new: true }
        );
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Ai5Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Ai5Form.findByIdAndDelete(req.params.id);
