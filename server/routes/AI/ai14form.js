...
const Ai14Form = require('../../models/AI/Ai14Form'); // Ensure correct model path

// ✅ POST route for AI14 form submission
router.post('/', async (req, res) => {
    ...
    try {
        ...
        await newForm.save();
        res.status(201).json({ message: 'AI14 Form submitted successfully!', newForm });
    } catch (error) {
        console.error('Error submitting AI14 form:', error.message);
        res.status(500).json({ message: 'Internal server error', error: error.message });
    }
});

// ✅ GET route to fetch all AI14 submissions
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Ai14Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
        return res.status(200).json(submissions);
    } catch (error) {
        console.error('Error fetching AI14 submissions:', error);
...
router.get('/check-submission', async (req, res) => {
    ...
    try {
        const existingSubmission = await Ai14Form.findOne({ teacherName, teacherDepartment });
...
// ✅ PUT route to update an existing AI14 submission
router.put('/:id', async (req, res) => {
    ...
    try {
        const updatedForm = await Ai14Form.findByIdAndUpdate(
            req.params.id,
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Ai14Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Ai14Form.findByIdAndDelete(req.params.id);
