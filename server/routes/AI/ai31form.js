...
const Ai31Form = require('../../models/AI/Ai31Form'); // Ensure correct model path

// ✅ POST route for AI3.1 form submission
router.post('/', async (req, res) => {
    const { teacherName, teacherDepartment, qualityOfSpeaker, feedbackReceived, numberOfAttendees, numberOfStudents, mapping, linkEvidence, formId, score } = req.body;

    if (!teacherName || !teacherDepartment || !qualityOfSpeaker || !feedbackReceived || !numberOfAttendees || !numberOfStudents || !mapping || !linkEvidence || !formId) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    try {
        const newForm = new Ai31Form({
            teacherName,
            teacherDepartment,
            qualityOfSpeaker,
            feedbackReceived,
...
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Ai31Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
...
router.put('/:id', async (req, res) => {
    const { qualityOfSpeaker, feedbackReceived, numberOfAttendees, numberOfStudents, mapping, linkEvidence, score } = req.body;

    if (!qualityOfSpeaker || !feedbackReceived || !numberOfAttendees || !numberOfStudents || !mapping || !linkEvidence) {
        return res.status(400).json({ message: 'All fields are required to update the form' });
    }

    try {
        const updatedForm = await Ai31Form.findByIdAndUpdate(
            req.params.id,
            { qualityOfSpeaker, feedbackReceived, numberOfAttendees, numberOfStudents, mapping, linkEvidence, score },
            { new: true }
        );
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Ai31Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Ai31Form.findByIdAndDelete(req.params.id);
