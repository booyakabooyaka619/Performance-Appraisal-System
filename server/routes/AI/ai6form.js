...
const Ai6Form = require('../../models/AI/Ai6Form'); // Ensure correct model path

router.post('/', async (req, res) => {
    const {
        teacherName, teacherDepartment, formId,
        numStudentsInnovativeTLP, numStudentsEnrollment,
        qualityAssignments, qualityQuizzes, qualityExperiment,
        engagementActivities, slowLearnerActivities, advanceLearnerActivities,
        numAssessmentActivities, mapping, linkEvidence, score
    } = req.body;
    ...
    if (
        ...
        !qualityAssignments || !qualityQuizzes || !qualityExperiment ||
        !engagementActivities || !slowLearnerActivities || !advanceLearnerActivities ||
        !numAssessmentActivities || !mapping || !linkEvidence
    ) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    try {
        const newForm = new Ai6Form({
            teacherName, teacherDepartment, formId,
            numStudentsInnovativeTLP, numStudentsEnrollment,
            qualityAssignments, qualityQuizzes, qualityExperiment,
            engagementActivities, slowLearnerActivities, advanceLearnerActivities,
            numAssessmentActivities, mapping, linkEvidence, score: score || 0
        });

        await newForm.save();
        res.status(201).json({ message: 'Ai6 Form submitted successfully!', newForm });
    } catch (error) {
        console.error('Error submitting Ai6 form:', error.message);
        res.status(500).json({ message: 'Internal server error', error: error.message });
    }
});

// ✅ GET route to fetch all Ai6 submissions
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Ai6Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
        return res.status(200).json(submissions);
    } catch (error) {
        console.error('Error fetching AI6 submissions:', error);
...
router.get('/check-submission', async (req, res) => {
    ...
    try {
        const existingSubmission = await Ai6Form.findOne({ teacherName, teacherDepartment });
...
// ✅ PUT route to update an existing Ai6 submission
router.put('/:id', async (req, res) => {
    ...
    try {
        const updatedForm = await Ai6Form.findByIdAndUpdate(
            req.params.id,
            {
                numStudentsInnovativeTLP, numStudentsEnrollment,
                qualityAssignments, qualityQuizzes, qualityExperiment,
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Ai6Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Ai6Form.findByIdAndDelete(req.params.id);
