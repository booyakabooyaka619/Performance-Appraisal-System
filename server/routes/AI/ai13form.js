...
const Ai13Form = require('../../models/AI/Ai13Form'); // Ensure correct model path

// ✅ POST route for AI13 form submission
router.post('/', async (req, res) => {
    const { teacherName, teacherDepartment, formId, previewGrade, linkEvidence, score } = req.body;

    if (!teacherName || !teacherDepartment || !formId || !previewGrade || !linkEvidence) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    try {
        const newForm = new Ai13Form({
            teacherName,
            teacherDepartment,
            formId,
            previewGrade,
        ...
        await newForm.save();
        res.status(201).json({ message: 'AI13 Form submitted successfully!', newForm });
    } catch (error) {
        console.error('Error submitting AI13 form:', error.message);
        res.status(500).json({ message: 'Internal server error', error: error.message });
    }
});

// ✅ GET route to fetch all AI13 submissions
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Ai13Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
        return res.status(200).json(submissions);
    } catch (error) {
        console.error('Error fetching AI13 submissions:', error);
...
router.get('/check-submission', async (req, res) => {
    ...
    try {
        const existingSubmission = await Ai13Form.findOne({ teacherName, teacherDepartment });
...
// ✅ PUT route to update an existing AI13 submission
router.put('/:id', async (req, res) => {
    const { previewGrade, linkEvidence, score } = req.body;

    if (!previewGrade || !linkEvidence) {
        return res.status(400).json({ message: 'All fields are required to update the form' });
    }

    try {
        const updatedForm = await Ai13Form.findByIdAndUpdate(
            req.params.id,
            { previewGrade, linkEvidence, score },
            { new: true }
        );
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Ai13Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Ai13Form.findByIdAndDelete(req.params.id);
