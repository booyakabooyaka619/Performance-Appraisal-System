...
const Cb2Form = require('../../models/CB/Cb2Form'); // Ensure correct model path

// ✅ POST route for CB2 form submission
router.post('/', async (req, res) => {
    const { teacherName, teacherDepartment, expertAICTE, expertDTE, academicCouncil, linkEvidence, formId, score } = req.body;

    if (!teacherName || !teacherDepartment || !expertAICTE || !expertDTE || !academicCouncil || !linkEvidence || !formId) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    try {
        const newForm = new Cb2Form({
            teacherName,
            teacherDepartment,
            expertAICTE,
            expertDTE,
        ...
        await newForm.save();
        res.status(201).json({ message: 'CB2 Form submitted successfully!', newForm });
    } catch (error) {
        console.error('Error submitting CB2 form:', error.message);
        res.status(500).json({ message: 'Internal server error', error: error.message });
    }
});

// ✅ GET route to fetch all CB2 submissions
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Cb2Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
        return res.status(200).json(submissions);
    } catch (error) {
        console.error('Error fetching CB2 submissions:', error);
...
router.get('/check-submission', async (req, res) => {
    ...
    try {
        const existingSubmission = await Cb2Form.findOne({ teacherName, teacherDepartment });
...
// ✅ PUT route to update an existing CB2 submission
router.put('/:id', async (req, res) => {
    const { expertAICTE, expertDTE, academicCouncil, linkEvidence, score } = req.body;

    if (!expertAICTE || !expertDTE || !academicCouncil || !linkEvidence) {
        return res.status(400).json({ message: 'All fields are required to update the form' });
    }

    try {
        const updatedForm = await Cb2Form.findByIdAndUpdate(
            req.params.id,
            { expertAICTE, expertDTE, academicCouncil, linkEvidence, score },
            { new: true }
        );
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Cb2Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Cb2Form.findByIdAndDelete(req.params.id);
