...
const Cb3Form = require('../../models/CB/Cb3Form'); // ✅ Adjust the model path if necessary

// ✅ POST route for Cb3 form submission
router.post('/', async (req, res) => {
    const { teacherName, teacherDepartment, revenueAmount, teamPosition, linkEvidence, formId, score } = req.body;

    if (!teacherName || !teacherDepartment || !revenueAmount || !teamPosition || !linkEvidence || !formId) {
        return res.status(400).json({ message: 'All fields are required' });
    }

    try {
        const newForm = new Cb3Form({
            teacherName,
            teacherDepartment,
            revenueAmount,
            teamPosition,
        ...
        await newForm.save();
        res.status(201).json({ message: 'Cb3 Form submitted successfully!', newForm });
    } catch (error) {
        console.error('Error submitting Cb3 form:', error.message);
        res.status(500).json({ message: 'Internal server error', error: error.message });
    }
});

// ✅ GET route to fetch all Cb3 submissions
router.get('/submissions', async (req, res) => {
    ...
    try {
        const submissions = await Cb3Form.find({ teacherName, teacherDepartment, formId }).sort({ createdAt: -1 });
        return res.status(200).json(submissions);
    } catch (error) {
        console.error('Error fetching Cb3 submissions:', error);
...
router.get('/check-submission', async (req, res) => {
    ...
    try {
        const existingSubmission = await Cb3Form.findOne({ teacherName, teacherDepartment });
...
// ✅ PUT route to update an existing Cb3 submission
router.put('/:id', async (req, res) => {
    const { revenueAmount, teamPosition, linkEvidence, score } = req.body;

    if (!revenueAmount || !teamPosition || !linkEvidence) {
        return res.status(400).json({ message: 'All fields are required to update the form' });
    }

    try {
        const updatedForm = await Cb3Form.findByIdAndUpdate(
            req.params.id,
            { revenueAmount, teamPosition, linkEvidence, score },
            { new: true }
        );
...
router.delete('/:id', async (req, res) => {
    try {
        const form = await Cb3Form.findById(req.params.id);
        if (!form) {
            return res.status(404).json({ message: 'Form not found' });
        }

        await Cb3Form.findByIdAndDelete(req.params.id);
