...
export default function RORB6() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherRB6Forms, setTeacherRB6Forms] = useState([]);
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/rb6form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=RB6`);
    ...
    const fetchTeacherRB6Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb6form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=RB6`);
            if (!response.ok) throw new Error('Failed to fetch RB6 form');
            const result = await response.json();

            setTeacherRB6Forms(result.reverse());
            ...
            console.error('Error fetching teacher RB6 form:', error);
        }
    };

    useEffect(() => {
        if (teacherRB6Forms.length > 0) {
            const form = teacherRB6Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb6form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/rb6form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#f15b4c ' }}>RB-6 Special Honours/Awards/Recognitions/Post-doctoral fellowships
                </h2>

                {/* Teacher RB6 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherRB6Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherRB6Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherRB6Forms.length === 0 ? (
                        <p className='noprev'>No previous RB6 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherRB6Forms[currentPage - 1]?.awardType}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherRB6Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherRB6Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherRB6Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                                ...
                                                <Modal.Body>
                                                    <ul>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            The score is calculated based on the level at which the award or recognition is received.
                                                            Each level is assigned a weight:
                                                            International, National, State, Intercollegiate, and Other.
                                                        </li>
                                                        <li>
                                                            The assigned multiplier is then multiplied by 50 to calculate the final score.
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            <strong>Example:</strong> If the selected award type is:<br />
                                                            <code>National (1.8)</code>
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            Then Final Score = <code>1.8 × 50 = 90</code>
                                                        </li>
                                                        <li>
                                                            Awards received at higher levels (e.g., International) yield higher scores.
                                                        </li>
                                                    </ul>
                                                </Modal.Body>
                                                <Modal.Footer>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherRB6Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherRB6Forms[currentPage - 1]._id)}
