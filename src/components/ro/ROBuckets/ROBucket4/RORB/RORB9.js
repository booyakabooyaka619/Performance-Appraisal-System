...
export default function RORB9() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherRB9Forms, setTeacherRB9Forms] = useState([]);
    ...
    const fetchTeacherRB9Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb9form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=RB9`);
            if (!response.ok) throw new Error('Failed to fetch RB9 form');
            const result = await response.json();

            setTeacherRB9Forms(result.reverse());
            ...
            console.error('Error fetching teacher RB9 form:', error);
        }
    };

    useEffect(() => {
        if (teacherRB9Forms.length > 0) {
            const form = teacherRB9Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb9form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/rb9form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            <div className="bt-container">
                ...
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherRB9Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherRB9Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherRB9Forms.length === 0 ? (
                        <p className='noprev'>No previous RB9 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherRB9Forms[currentPage - 1]?.technicalContent}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Presentation Skills:</strong></td>
                                        <td>{teacherRB9Forms[currentPage - 1]?.presentationSkills}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>New/emerging/cutting edge Topic:</strong></td>
                                        <td>{teacherRB9Forms[currentPage - 1]?.newTopic}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Literature Survey:</strong></td>
                                        <td>{teacherRB9Forms[currentPage - 1]?.literatureSurvey}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Research Aspects:</strong></td>
                                        <td>{teacherRB9Forms[currentPage - 1]?.researchAspects}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Audience Interaction:</strong></td>
                                        <td>{teacherRB9Forms[currentPage - 1]?.audienceInteraction}</td>
                                    </tr>

                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherRB9Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherRB9Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherRB9Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherRB9Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherRB9Forms[currentPage - 1]._id)}
