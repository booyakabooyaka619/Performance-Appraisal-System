...
export default function RORB3() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherRB3Forms, setTeacherRB3Forms] = useState([]);
    ...
    const fetchTeacherRB3Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb3form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=RB3`);
            if (!response.ok) throw new Error('Failed to fetch RB3 form');
            const result = await response.json();

            setTeacherRB3Forms(result.reverse());
            ...
            console.error('Error fetching teacher RB3 form:', error);
        }
    };

    useEffect(() => {
        if (teacherRB3Forms.length > 0) {
            const form = teacherRB3Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb3form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/rb3form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            <div className="bt-container">
                ...
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherRB3Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherRB3Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherRB3Forms.length === 0 ? (
                        <p className='noprev'>No previous RB3 submissions found for the selected teacher.</p>
                    ) : (
                        <div className="submitted-form">
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr><td><strong>Quality of Conference:</strong></td><td>{teacherRB3Forms[currentPage - 1]?.qualityOfConference}</td></tr>
                                    <tr><td><strong>Level:</strong></td><td>{teacherRB3Forms[currentPage - 1]?.level}</td></tr>
                                    <tr><td><strong>Amount of Waiver:</strong></td><td>{teacherRB3Forms[currentPage - 1]?.waiverAmount}</td></tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherRB3Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherRB3Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherRB3Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherRB3Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherRB3Forms[currentPage - 1]._id)}
