...
export default function RORB4() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherRB4Forms, setTeacherRB4Forms] = useState([]);
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/rb4form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=RB4`);
    ...
    const fetchTeacherRB4Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb4form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=RB4`);
            if (!response.ok) throw new Error('Failed to fetch RB4 form');
            const result = await response.json();

            setTeacherRB4Forms(result.reverse());
            ...
            console.error('Error fetching teacher RB4 form:', error);
        }
    };

    useEffect(() => {
        if (teacherRB4Forms.length > 0) {
            const form = teacherRB4Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb4form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/rb4form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#f15b4c ' }}>RB-4 Patents/ Designs/Copyrights Or Commercialized Technologies
                </h2>

                {/* Teacher RB4 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherRB4Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherRB4Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherRB4Forms.length === 0 ? (
                        <p className='noprev'>No previous RB4 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherRB4Forms[currentPage - 1]?.status}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherRB4Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherRB4Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherRB4Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                                ...
                                                <Modal.Body>
                                                    <ul>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            The score is calculated by selecting the appropriate status from the dropdown:
                                                            <strong> Received Indian Patent, Received Foreign Patent, Copyrights, or Filed.</strong>
                                                        </li>
                                                        <li>
                                                            Each status carries a different multiplier value. The selected value is multiplied by 200 to compute the final score.
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            <strong>Example:</strong> If the selected status is:<br />
                                                            <code>Received Indian patent = 1.25</code>
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            Then Final Score = <code>1.25 × 200 = 250</code>
                                                        </li>
                                                        <li>
                                                            Higher achievements (like granted patents) result in a higher score, while "Filed" carries a lower score weight.
                                                        </li>
                                                    </ul>
                                                </Modal.Body>
                                                <Modal.Footer>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherRB4Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherRB4Forms[currentPage - 1]._id)}
