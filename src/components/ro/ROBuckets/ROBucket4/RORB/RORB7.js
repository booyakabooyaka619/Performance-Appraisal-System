...
export default function RORB7() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherRB7Forms, setTeacherRB7Forms] = useState([]);
    ...
    const fetchTeacherRB7Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb7form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=RB7`);
            if (!response.ok) throw new Error('Failed to fetch RB7 form');
            const result = await response.json();

            setTeacherRB7Forms(result.reverse());
            ...
            console.error('Error fetching teacher RB7 form:', error);
        }
    };

    useEffect(() => {
        if (teacherRB7Forms.length > 0) {
            const form = teacherRB7Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb7form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/rb7form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            <div className="bt-container">
                ...
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherRB7Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherRB7Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherRB7Forms.length === 0 ? (
                        <p className='noprev'>No previous RB7 submissions found for the selected teacher.</p>
                    ) : (
                        <div className="submitted-form">
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr><td><strong>Publisher Type:</strong></td><td>{teacherRB7Forms[currentPage - 1]?.publisherType}</td></tr>
                                    <tr><td><strong>Publication Type:</strong></td><td>{teacherRB7Forms[currentPage - 1]?.publicationType}</td></tr>
                                    <tr><td><strong>Authorship:</strong></td><td>{teacherRB7Forms[currentPage - 1]?.authorship}</td></tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherRB7Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherRB7Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherRB7Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherRB7Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherRB7Forms[currentPage - 1]._id)}
