...
export default function RORB8() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherRB8Forms, setTeacherRB8Forms] = useState([]);
    ...
    const fetchTeacherRB8Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb8form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=RB8`);
            if (!response.ok) throw new Error('Failed to fetch RB8 form');
            const result = await response.json();

            setTeacherRB8Forms(result.reverse());
            ...
            console.error('Error fetching teacher RB8 form:', error);
        }
    };

    useEffect(() => {
        if (teacherRB8Forms.length > 0) {
            const form = teacherRB8Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb8form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/rb8form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            <div className="bt-container">
                ...
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherRB8Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherRB8Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherRB8Forms.length === 0 ? (
                        <p className='noprev'>No previous RB8 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherRB8Forms[currentPage - 1]?.organisingAgency}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Quality of Speaker:</strong></td>
                                        <td>{teacherRB8Forms[currentPage - 1]?.speakerQuality}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Duration:</strong></td>
                                        <td>{teacherRB8Forms[currentPage - 1]?.duration}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Mode of Conduct:</strong></td>
                                        <td>{teacherRB8Forms[currentPage - 1]?.modeOfConduct}</td>
                                    </tr>

                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherRB8Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherRB8Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherRB8Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherRB8Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherRB8Forms[currentPage - 1]._id)}
