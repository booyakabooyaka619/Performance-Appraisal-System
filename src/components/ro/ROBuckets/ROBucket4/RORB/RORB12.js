...
export default function RORB12() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherRB12Forms, setTeacherRB12Forms] = useState([]);
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/rb12form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=RB12`);
    ...
    const fetchTeacherRB12Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb12form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=RB12`);
            if (!response.ok) throw new Error('Failed to fetch RB12 form');
            const result = await response.json();

            setTeacherRB12Forms(result.reverse());
            ...
            console.error('Error fetching teacher RB12 form:', error);
        }
    };

    useEffect(() => {
        if (teacherRB12Forms.length > 0) {
            const form = teacherRB12Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb12form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/rb12form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#f15b4c' }}>RB-12 Session Chair / Reviewer
                </h2>

                {/* Teacher RB12 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherRB12Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherRB12Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherRB12Forms.length === 0 ? (
                        <p className='noprev'>No previous RB12 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherRB12Forms[currentPage - 1]?.activityType}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Level:</strong></td>
                                        <td>{teacherRB12Forms[currentPage - 1]?.activityLevel}</td>
                                    </tr>

                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherRB12Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherRB12Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherRB12Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                                ...
                                                <Modal.Body>
                                                    <ul>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            The score is calculated by multiplying the selected values from both dropdowns:
                                                            <strong> Activity Type</strong> and <strong>Level</strong>.
                                                        </li>
                                                        <li>
                                                            After multiplying the selected values, the result is multiplied by 50 to get the final score.
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            <strong>Example:</strong> If the selected values are:<br />
                                                            Activity Type = 1.25 (e.g., Thesis Evaluation – Ph.D.)<br />
                                                            Level = 1.0 (e.g., National)
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            Then Final Score = <code>1.25 × 1.0 × 50 = 62.5</code>
                                                        </li>
                                                        <li>
                                                            Selecting higher-valued activities and levels results in a higher score.
                                                        </li>
                                                    </ul>
                                                </Modal.Body>
                                                <Modal.Footer>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherRB12Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherRB12Forms[currentPage - 1]._id)}
