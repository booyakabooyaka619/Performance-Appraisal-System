...
export default function RORB10() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherRB10Forms, setTeacherRB10Forms] = useState([]);
    ...
    const fetchTeacherRB10Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb10form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=RB10`);
            if (!response.ok) throw new Error('Failed to fetch RB10 form');
            const result = await response.json();

            setTeacherRB10Forms(result.reverse());
            ...
            console.error('Error fetching teacher RB10 form:', error);
        }
    };

    useEffect(() => {
        if (teacherRB10Forms.length > 0) {
            const form = teacherRB10Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb10form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/rb10form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            <div className="bt-container">
                ...
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherRB10Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherRB10Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherRB10Forms.length === 0 ? (
                        <p className='noprev'>No previous RB10 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherRB10Forms[currentPage - 1]?.typeOfInteraction}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Type of Partner:</strong></td>
                                        <td>{teacherRB10Forms[currentPage - 1]?.typeOfPartner}</td>
                                    </tr>

                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherRB10Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherRB10Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherRB10Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherRB10Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherRB10Forms[currentPage - 1]._id)}
