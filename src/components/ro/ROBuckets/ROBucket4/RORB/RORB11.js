...
export default function RORB11() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherRB11Forms, setTeacherRB11Forms] = useState([]);
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/rb11form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=RB11`);
    ...
    const fetchTeacherRB11Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb11form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=RB11`);
            if (!response.ok) throw new Error('Failed to fetch RB11 form');
            const result = await response.json();

            setTeacherRB11Forms(result.reverse());
            ...
            console.error('Error fetching teacher RB11 form:', error);
        }
    };

    useEffect(() => {
        if (teacherRB11Forms.length > 0) {
            const form = teacherRB11Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb11form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/rb11form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#f15b4c' }}>RB-11 Resource Person
                </h2>

                {/* Teacher RB11 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherRB11Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherRB11Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherRB11Forms.length === 0 ? (
                        <p className='noprev'>No previous RB11 submissions found for the selected teacher.</p>
                    ) : (
                        <div className="submitted-form">
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        <td><strong>Quality of Speakers:</strong></td>
                                        <td>{teacherRB11Forms[currentPage - 1]?.qualityOfSpeakers}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Organising Agency:</strong></td>
                                        <td>{teacherRB11Forms[currentPage - 1]?.organisingAgency}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Level:</strong></td>
                                        <td>{teacherRB11Forms[currentPage - 1]?.eventLevel}</td>
                                    </tr>

                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherRB11Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherRB11Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherRB11Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                                ...
                                                <Modal.Body>
                                                    <ul>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            ...
                                                            <strong> Quality of Speakers, Organising Agency, and Level of Event</strong>.
                                                        </li>
                                                        <li>
                                                            The resulting product is then multiplied by <strong>50</strong> to get the final score.
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            <strong>Example:</strong> If the selected values are:<br />
                                                            Quality of Speakers = 1.5, Organising Agency = 1.75, Level = 1.25
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            Then Final Score = <code>1.5 × 1.75 × 1.25 × 50 = 164.06</code>
                                                        </li>
                                                        <li>
                                                            Higher values in dropdown selections result in a higher final score.
                                                        </li>
                                                    </ul>
                                                </Modal.Body>
                                                <Modal.Footer>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherRB11Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherRB11Forms[currentPage - 1]._id)}
