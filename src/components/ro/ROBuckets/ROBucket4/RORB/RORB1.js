...
export default function RORB1() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherRB1Forms, setTeacherRB1Forms] = useState([]);
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/rb1form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=RB1`);
    ...
    const fetchTeacherRB1Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb1form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=RB1`);
            if (!response.ok) throw new Error('Failed to fetch RB1 form');
            const result = await response.json();

            setTeacherRB1Forms(result.reverse());
            ...
            console.error('Error fetching teacher RB1 form:', error);
        }
    };

    useEffect(() => {
        if (teacherRB1Forms.length > 0) {
            const form = teacherRB1Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb1form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/rb1form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#f15b4c ' }}>RB-1 Papers Publication in Conference
                </h2>

                {/* Teacher RB1 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherRB1Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherRB1Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherRB1Forms.length === 0 ? (
                        <p className='noprev'>No previous RB1 submissions found for the selected teacher.</p>
                    ) : (
                        <div className="submitted-form">
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr><td><strong>Quality of Conference:</strong></td><td>{teacherRB1Forms[currentPage - 1]?.conferenceQuality}</td></tr>
                                    <tr><td><strong>Level:</strong></td><td>{teacherRB1Forms[currentPage - 1]?.level}</td></tr>
                                    <tr><td><strong>Authorship:</strong></td><td>{teacherRB1Forms[currentPage - 1]?.authorship}</td></tr>
                                    <tr><td><strong>Type of Review:</strong></td><td>{teacherRB1Forms[currentPage - 1]?.reviewType}</td></tr>
                                    <tr><td><strong>Type of Presentation:</strong></td><td>{teacherRB1Forms[currentPage - 1]?.presentationType}</td></tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherRB1Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherRB1Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherRB1Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                                ...
                                                <Modal.Body>
                                                    <ul>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            ...
                                                            <strong> Conference Quality, Level, Authorship, Type of Review, and Type of Presentation</strong>.
                                                        </li>
                                                        <li>
                                                            After multiplying all the values together, the result is multiplied by <strong>50</strong> to get the final score.
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            <strong>Example:</strong> If the selected values are:<br />
                                                            Conference Quality = 1.5,<br />
                                                            Level = 1.25,<br />
                                                            Authorship = 1,<br />
                                                            Type of Review = 1,<br />
                                                            Type of Presentation = 1
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            Then Final Score = <code>1.5 × 1.25 × 1 × 1 × 1 × 50 = 93.75</code>
                                                        </li>
                                                        <li>
                                                            Choosing higher-quality options in each category will result in a higher score.
                                                        </li>
                                                    </ul>
                                                </Modal.Body>
                                                <Modal.Footer>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherRB1Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherRB1Forms[currentPage - 1]._id)}
