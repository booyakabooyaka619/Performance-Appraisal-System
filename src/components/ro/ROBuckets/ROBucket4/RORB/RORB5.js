...
export default function RORB5() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherRB5Forms, setTeacherRB5Forms] = useState([]);
    ...
    const fetchTeacherRB5Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb5form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=RB5`);
            if (!response.ok) throw new Error('Failed to fetch RB5 form');
            const result = await response.json();

            setTeacherRB5Forms(result.reverse());
            ...
            console.error('Error fetching teacher RB5 form:', error);
        }
    };

    useEffect(() => {
        if (teacherRB5Forms.length > 0) {
            const form = teacherRB5Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb5form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/rb5form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            <div className="bt-container">
                ...
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherRB5Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherRB5Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherRB5Forms.length === 0 ? (
                        <p className='noprev'>No previous RB5 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherRB5Forms[currentPage - 1]?.universityQuality}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Level of Qualification:</strong></td>
                                        <td>{teacherRB5Forms[currentPage - 1]?.qualificationLevel}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Grades Achieved:</strong></td>
                                        <td>{teacherRB5Forms[currentPage - 1]?.gradeAchieved}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherRB5Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherRB5Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherRB5Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherRB5Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherRB5Forms[currentPage - 1]._id)}
