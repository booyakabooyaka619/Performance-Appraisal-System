...
export default function ROAB8() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherAB8Forms, setTeacherAB8Forms] = useState([]);
    ...
    const fetchTeacherAB8Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ab8form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=AB8`);
            if (!response.ok) throw new Error('Failed to fetch AB8 form');
            const result = await response.json();

            setTeacherAB8Forms(result.reverse());
            ...
            console.error('Error fetching teacher AB8 form:', error);
        }
    };

    useEffect(() => {
        if (teacherAB8Forms.length > 0) {
            const form = teacherAB8Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/ab8form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/ab8form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            <div className="bt-container">
                ...
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherAB8Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherAB8Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherAB8Forms.length === 0 ? (
                        <p className='noprev'>No previous AB8 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherAB8Forms[currentPage - 1]?.typeOfMembership}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Position Held:</strong></td>
                                        <td>{teacherAB8Forms[currentPage - 1]?.positionHeld}</td>
                                    </tr>

                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherAB8Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherAB8Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherAB8Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherAB8Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherAB8Forms[currentPage - 1]._id)}
