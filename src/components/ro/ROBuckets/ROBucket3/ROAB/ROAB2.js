...
export default function ROAB2() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherAB2Forms, setTeacherAB2Forms] = useState([]);
    ...
    const fetchTeacherAB2Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ab2form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=AB2`);
            if (!response.ok) throw new Error('Failed to fetch AB2 form');
            const result = await response.json();

            setTeacherAB2Forms(result.reverse());
            ...
            console.error('Error fetching teacher AB2 form:', error);
        }
    };

    useEffect(() => {
        if (teacherAB2Forms.length > 0) {
            const form = teacherAB2Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/ab2form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/ab2form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            <div className="bt-container">
                ...
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherAB2Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherAB2Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherAB2Forms.length === 0 ? (
                        <p className='noprev'>No previous AB2 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherAB2Forms[currentPage - 1]?.registeredStudents}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Activity Duration:</strong></td>
                                        <td>{teacherAB2Forms[currentPage - 1]?.activityDuration}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Student Feedback:</strong></td>
                                        <td>{teacherAB2Forms[currentPage - 1]?.studentFeedback}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherAB2Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherAB2Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherAB2Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherAB2Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherAB2Forms[currentPage - 1]._id)}
