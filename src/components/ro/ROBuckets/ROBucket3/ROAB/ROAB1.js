...
export default function ROAB1() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherAB1Forms, setTeacherAB1Forms] = useState([]);
    ...
    const fetchTeacherAB1Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ab1form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=AB1`);
            if (!response.ok) throw new Error('Failed to fetch AB1 form');
            const result = await response.json();

            setTeacherAB1Forms(result.reverse());
            ...
            console.error('Error fetching teacher AB1 form:', error);
        }
    };

    useEffect(() => {
        if (teacherAB1Forms.length > 0) {
            const form = teacherAB1Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/ab1form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/ab1form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            <div className="bt-container">
                ...
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherAB1Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherAB1Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherAB1Forms.length === 0 ? (
                        <p className='noprev'>No previous AB1 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherAB1Forms[currentPage - 1]?.qualityActivity}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Paper Selection Ratio:</strong></td>
                                        <td>{teacherAB1Forms[currentPage - 1]?.paperSelectionRatio}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Association:</strong></td>
                                        <td>{teacherAB1Forms[currentPage - 1]?.association}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Designation:</strong></td>
                                        <td>{teacherAB1Forms[currentPage - 1]?.designation}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherAB1Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherAB1Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherAB1Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherAB1Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherAB1Forms[currentPage - 1]._id)}
