...
export default function ROAB3() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherAB3Forms, setTeacherAB3Forms] = useState([]);
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/ab3form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=AB3`);
    ...
    const fetchTeacherAB3Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ab3form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=AB3`);
            if (!response.ok) throw new Error('Failed to fetch AB3 form');
            const result = await response.json();

            setTeacherAB3Forms(result.reverse());
            ...
            console.error('Error fetching teacher AB3 form:', error);
        }
    };

    useEffect(() => {
        if (teacherAB3Forms.length > 0) {
            const form = teacherAB3Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/ab3form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/ab3form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#cc222b' }}>AB-3 Co-curricular Activities: Extension and Field-based Activities
                </h2>

                {/* Teacher AB3 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherAB3Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherAB3Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherAB3Forms.length === 0 ? (
                        <p className='noprev'>No previous AB3 submissions found for the selected teacher.</p>
                    ) : (
                        <div className="submitted-form">
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        <td><strong>Students Participated:</strong></td>
                                        <td>{teacherAB3Forms[currentPage - 1]?.studentsParticipated}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Activity Duration:</strong></td>
                                        <td>{teacherAB3Forms[currentPage - 1]?.activityDuration}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Feedback Rating:</strong></td>
                                        <td>{teacherAB3Forms[currentPage - 1]?.feedback}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherAB3Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherAB3Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherAB3Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                                ...
                                                <Modal.Body>
                                                    <ul>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            ...
                                                            Number of Students Participated, Activity Duration, and Students' Feedback.
                                                        </li>
                                                        <li>
                                                        ...
                                                        <li style={{ marginBottom: '20px' }}>
                                                            ...
                                                            Students Participated = 50+ (1.25), Activity Duration = Two weeks (1.75), Students' Feedback = 3.0+ (1)
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            Then Final Score = <code>1.25 × 1.75 × 1 × 100 = 218.75</code>
                                                        </li>
                                                        <li>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherAB3Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherAB3Forms[currentPage - 1]._id)}
