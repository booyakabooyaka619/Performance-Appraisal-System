...
export default function ROCB1() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherCB1Forms, setTeacherCB1Forms] = useState([]);
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/cb1form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=CB1`);
    ...
    const fetchTeacherCB1Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/cb1form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=CB1`);
            if (!response.ok) throw new Error('Failed to fetch CB1 form');
            const result = await response.json();

            setTeacherCB1Forms(result.reverse());
            ...
            console.error('Error fetching teacher CB1 form:', error);
        }
    };

    useEffect(() => {
        if (teacherCB1Forms.length > 0) {
            const form = teacherCB1Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/cb1form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/cb1form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#faa41b' }}>CB-1 Internal Revenue Generation (IRG)
                </h2>

                {/* Teacher CB1 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherCB1Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherCB1Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherCB1Forms.length === 0 ? (
                        <p className='noprev'>No previous CB1 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherCB1Forms[currentPage - 1]?.revenueGenerated}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Consultancy Position:</strong></td>
                                        <td>{teacherCB1Forms[currentPage - 1]?.consultancyPosition}</td>
                                    </tr>

                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherCB1Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherCB1Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherCB1Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                                ...
                                                <Modal.Body>
                                                    <ul>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            The score is calculated by multiplying the values selected in the following two categories:
                                                            <strong> Amount of Revenue Generated</strong> and <strong>Consultancy Position</strong>.
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            After obtaining the product of the two selected values, the result is multiplied by 375 to get the final score.
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            <strong>Example:</strong> If the selected values are:<br />
                                                            Amount of Revenue Generated = 1.25<br />
                                                            Consultancy Position = 0.8
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            Then Final Score = <code>1.25 × 0.8 × 375 = 375</code>
                                                        </li>
                                                        <li>
                                                            Higher revenue and higher roles lead to higher scores.
                                                        </li>
                                                    </ul>
                                                </Modal.Body>
                                                <Modal.Footer>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherCB1Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherCB1Forms[currentPage - 1]._id)}
