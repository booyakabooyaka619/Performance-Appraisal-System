...
export default function ROAI13() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherAI13Forms, setTeacherAI13Forms] = useState([]);
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/ai13form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=AI13`);
    ...
    const fetchTeacherAI13Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai13form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=AI13`);
            if (!response.ok) throw new Error('Failed to fetch AI13 form');
            const result = await response.json();

            setTeacherAI13Forms(result.reverse());
            ...
            console.error('Error fetching teacher AI13 form:', error);
        }
    };

    useEffect(() => {
        if (teacherAI13Forms.length > 0) {
            const form = teacherAI13Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai13form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/ai13form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#537c78' }}>AI-13 Preview Grades (Preceding Semester)</h2>

                {/* Teacher AI13 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherAI13Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherAI13Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherAI13Forms.length === 0 ? (
                        <p className='noprev'>No previous AI13 submissions found for the selected teacher.</p>
                    ) : (
                        <div className="submitted-form">
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        <td><strong>Preview Grade:</strong></td>
                                        <td>{teacherAI13Forms[currentPage - 1]?.previewGrade}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherAI13Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Evidence Link:</strong></td>
                                        <td>
                                            <a
                                                href={teacherAI13Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherAI13Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                                ...
                                                <Modal.Body>
                                                    <ul>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            The score is based on the selected <strong className="flex-strong">Review Grade</strong> awarded to the faculty.
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            Each grade corresponds to a specific score as per the quality benchmark:
                                                            <ul style={{ marginTop: '10px' }}>
                                                                <li><strong className="flex-strong">A</strong> = 100 marks</li>
                                                                <li><strong className="flex-strong">B</strong> = 50 marks</li>
                                                                <li><strong className="flex-strong">C</strong> = 25 marks</li>
                                                                <li><strong className="flex-strong">Below C</strong> = 0 marks</li>
                                                            </ul>
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            <strong className="flex-strong">Example:</strong> If the faculty received grade B, the score awarded will be 75.
                                                        </li>
                                                        <li>
                                                            Higher grades result in higher scores.
                                                        </li>
                                                    </ul>
                                                </Modal.Body>
                                                <Modal.Footer>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherAI13Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherAI13Forms[currentPage - 1]._id)}
