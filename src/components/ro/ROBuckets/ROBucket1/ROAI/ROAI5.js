...
export default function ROAI5() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherAI5Forms, setTeacherAI5Forms] = useState([]);
    ...
    const fetchTeacherAI5Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai5form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=AI5`);
            if (!response.ok) throw new Error('Failed to fetch AI5 form');
            const result = await response.json();

            setTeacherAI5Forms(result.reverse());
            ...
            console.error('Error fetching teacher AI5 form:', error);
        }
    };

    useEffect(() => {
        if (teacherAI5Forms.length > 0) {
            const form = teacherAI5Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai5form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/ai5form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#537c78' }}>AI-5 Course/ Lab CO/LO-Attainment</h2>

                {/* Teacher AI5 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherAI5Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherAI5Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherAI5Forms.length === 0 ? (
                        <p className='noprev'>No previous AI5 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherAI5Forms[currentPage - 1]?.courseLabName}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Attainment Level (0-3):</strong></td>
                                        <td>{teacherAI5Forms[currentPage - 1]?.attainmentLevel}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherAI5Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Evidence Link:</strong></td>
                                        <td>
                                            <a
                                                href={teacherAI5Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherAI5Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherAI5Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherAI5Forms[currentPage - 1]._id)}
