...
export default function ROAI7() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherAI7Forms, setTeacherAI7Forms] = useState([]);
    ...
    const fetchTeacherAI7Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai7form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=AI7`);
            if (!response.ok) throw new Error('Failed to fetch AI7 form');
            const result = await response.json();

            setTeacherAI7Forms(result.reverse());
            ...
            console.error('Error fetching teacher AI7 form:', error);
        }
    };

    useEffect(() => {
        if (teacherAI7Forms.length > 0) {
            const form = teacherAI7Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai7form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/ai7form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#537c78' }}>AI-7 Contribution for Learning Resources Development at Institute </h2>

                {/* Teacher AI7 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherAI7Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherAI7Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherAI7Forms.length === 0 ? (
                        <p className='noprev'>No previous AI7 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherAI7Forms[currentPage - 1]?.courseName}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Theory Notes Quality Factor (0-1):</strong></td>
                                        <td>{teacherAI7Forms[currentPage - 1]?.theoryNotes}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>E-Book Quality Factor (0-1):</strong></td>
                                        <td>{teacherAI7Forms[currentPage - 1]?.eBook}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Resource Book Quality Factor (0-1):</strong></td>
                                        <td>{teacherAI7Forms[currentPage - 1]?.resourceBook}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>AAP Validation Quality Factor (0-1):</strong></td>
                                        <td>{teacherAI7Forms[currentPage - 1]?.aapValidation}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherAI7Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Evidence Link:</strong></td>
                                        <td>
                                            <a
                                                href={teacherAI7Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherAI7Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherAI7Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherAI7Forms[currentPage - 1]._id)}
