...
export default function ROAI10() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherAI10Forms, setTeacherAI10Forms] = useState([]);
    ...
    const fetchTeacherAI10Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai10form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=AI10`);
            if (!response.ok) throw new Error('Failed to fetch AI10 form');
            const result = await response.json();

            setTeacherAI10Forms(result.reverse());
            ...
            console.error('Error fetching teacher AI10 form:', error);
        }
    };

    useEffect(() => {
        if (teacherAI10Forms.length > 0) {
            const form = teacherAI10Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai10form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/ai10form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            <div className="bt-container">
                ...
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherAI10Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherAI10Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherAI10Forms.length === 0 ? (
                        <p className='noprev'>No previous AI10 submissions found for the selected teacher.</p>
                    ) : (
                        <div className="submitted-form">
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr><td><strong>Number of Projects:</strong></td><td>{teacherAI10Forms[currentPage - 1]?.numberOfProjects}</td></tr>
                                    <tr><td><strong>Industry Projects:</strong></td><td>{teacherAI10Forms[currentPage - 1]?.industryProjects}</td></tr>
                                    <tr><td><strong>Live Projects:</strong></td><td>{teacherAI10Forms[currentPage - 1]?.liveProjects}</td></tr>
                                    <tr><td><strong>Participation in Competitions:</strong></td><td>{teacherAI10Forms[currentPage - 1]?.participationInCompetitions}</td></tr>
                                    <tr><td><strong>Awards:</strong></td><td>{teacherAI10Forms[currentPage - 1]?.awards}</td></tr>
                                    <tr><td><strong>Publications:</strong></td><td>{teacherAI10Forms[currentPage - 1]?.publications}</td></tr>
                                    <tr><td><strong>Funding Received:</strong></td><td>{teacherAI10Forms[currentPage - 1]?.fundingReceived}</td></tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherAI10Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Evidence Link:</strong></td>
                                        <td>
                                            <a href={teacherAI10Forms[currentPage - 1]?.linkEvidence} target="_blank" rel="noopener noreferrer">
                                                {teacherAI10Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherAI10Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherAI10Forms[currentPage - 1]._id)}
