...
export default function ROAI34() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherAI34Forms, setTeacherAI34Forms] = useState([]);
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/ai34form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=AI34`);
    ...
    const fetchTeacherAI34Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai34form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=AI34`);
            if (!response.ok) throw new Error('Failed to fetch AI34 form');
            const result = await response.json();

            setTeacherAI34Forms(result.reverse());
            ...
            console.error('Error fetching teacher AI34 form:', error);
        }
    };

    useEffect(() => {
        if (teacherAI34Forms.length > 0) {
            const form = teacherAI34Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai34form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/ai34form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#537c78' }}>AI-3.4 BSA - Mini Project (within a Course) </h2>

                {/* Teacher AI34 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherAI34Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherAI34Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherAI34Forms.length === 0 ? (
                        <p className='noprev'>No previous AI34 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherAI34Forms[currentPage - 1]?.typeOfGuide}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Type of Organisation:</strong></td>
                                        <td>{teacherAI34Forms[currentPage - 1]?.typeOfOrganisation}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Type of Project:</strong></td>
                                        <td>{teacherAI34Forms[currentPage - 1]?.typeOfProject}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Mapping:</strong></td>
                                        <td>{teacherAI34Forms[currentPage - 1]?.mapping}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherAI34Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Evidence Link:</strong></td>
                                        <td>
                                            <a
                                                href={teacherAI34Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherAI34Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                                ...
                                                <Modal.Body>
                                                    <ul>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            ...
                                                            Type of Guide, Type of Organisation of Industry Co-guide, Type of Project, and Mapping.
                                                        </li>
                                                        <li>
                                                            After multiplying all the values, the result is multiplied by 75 to get the final score.
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            <strong>Example:</strong> If the selected values are:<br />
                                                            Type of Guide = 1.5, Type of Organisation = 1.3, Type of Project = 1, Mapping = 1.5
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            Then Final Score = <code>1.5 × 1.3 × 1 × 1.5 × 75 = 146.25</code>
                                                        </li>
                                                        <li>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherAI34Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherAI34Forms[currentPage - 1]._id)}
