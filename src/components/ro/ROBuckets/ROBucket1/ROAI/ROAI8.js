...
export default function ROAI8() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherAI8Forms, setTeacherAI8Forms] = useState([]);
    ...
    const fetchTeacherAI8Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai8form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=AI8`);
            if (!response.ok) throw new Error('Failed to fetch AI8 form');
            const result = await response.json();

            setTeacherAI8Forms(result.reverse());
            ...
            console.error('Error fetching teacher AI8 form:', error);
        }
    };

    useEffect(() => {
        if (teacherAI8Forms.length > 0) {
            const form = teacherAI8Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai8form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/ai8form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            <div className="bt-container">
                ...
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherAI8Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherAI8Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherAI8Forms.length === 0 ? (
                        <p className='noprev'>No previous AI8 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherAI8Forms[currentPage - 1]?.courseName}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Type of Guide:</strong></td>
                                        <td>{teacherAI8Forms[currentPage - 1]?.typeOfGuide}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Type of Organisation of Industry Co-guide:</strong></td>
                                        <td>{teacherAI8Forms[currentPage - 1]?.typeOfOrganisation}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Type of Project:</strong></td>
                                        <td>{teacherAI8Forms[currentPage - 1]?.typeOfProject}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Publications:</strong></td>
                                        <td>{teacherAI8Forms[currentPage - 1]?.publications}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Mapping:</strong></td>
                                        <td>{teacherAI8Forms[currentPage - 1]?.mapping}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherAI8Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Evidence Link:</strong></td>
                                        <td>
                                            <a
                                                href={teacherAI8Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherAI8Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherAI8Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherAI8Forms[currentPage - 1]._id)}
