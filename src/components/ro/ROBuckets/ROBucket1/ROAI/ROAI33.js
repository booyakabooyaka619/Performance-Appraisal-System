...
export default function ROAI33() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherAI33Forms, setTeacherAI33Forms] = useState([]);
    ...
    const fetchTeacherAI33Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai33form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=AI33`);
            if (!response.ok) throw new Error('Failed to fetch AI33 form');
            const result = await response.json();

            setTeacherAI33Forms(result.reverse());
            ...
            console.error('Error fetching teacher AI33 form:', error);
        }
    };

    useEffect(() => {
        if (teacherAI33Forms.length > 0) {
            const form = teacherAI33Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai33form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/ai33form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            <div className="bt-container">
                ...
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherAI33Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherAI33Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherAI33Forms.length === 0 ? (
                        <p className='noprev'>No previous AI33 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherAI33Forms[currentPage - 1]?.typeOfEvent}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Awards Received:</strong></td>
                                        <td>{teacherAI33Forms[currentPage - 1]?.awardsReceived}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Feedback Received:</strong></td>
                                        <td>{teacherAI33Forms[currentPage - 1]?.feedbackReceived}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Number of Attendees (A):</strong></td>
                                        <td>{teacherAI33Forms[currentPage - 1]?.numberOfAttendees}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Total Students (T):</strong></td>
                                        <td>{teacherAI33Forms[currentPage - 1]?.numberOfStudents}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Mapping:</strong></td>
                                        <td>{teacherAI33Forms[currentPage - 1]?.mapping}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherAI33Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Evidence Link:</strong></td>
                                        <td>
                                            <a
                                                href={teacherAI33Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherAI33Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherAI33Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherAI33Forms[currentPage - 1]._id)}
