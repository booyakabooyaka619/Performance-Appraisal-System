...
export default function ROAI14() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherAI14Forms, setTeacherAI14Forms] = useState([]);
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/ai14form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=AI14`);
    ...
    const fetchTeacherAI14Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai14form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=AI14`);
            if (!response.ok) throw new Error('Failed to fetch AI14 form');
            const result = await response.json();

            setTeacherAI14Forms(result.reverse());
            ...
            console.error('Error fetching teacher AI14 form:', error);
        }
    };

    useEffect(() => {
        if (teacherAI14Forms.length > 0) {
            const form = teacherAI14Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai14form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/ai14form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#537c78' }}>AI-14 Review Grades (Preceding Semester)</h2>

                {/* Teacher AI14 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherAI14Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherAI14Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherAI14Forms.length === 0 ? (
                        <p className='noprev'>No previous AI14 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherAI14Forms[currentPage - 1]?.previewGrade}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherAI14Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Evidence Link:</strong></td>
                                        <td>
                                            <a
                                                href={teacherAI14Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherAI14Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                                    ...
                                                    <ul>
                                                        ...
                                                        <li style={{ marginBottom: '20px' }}>
                                                            ...
                                                            <ul style={{ marginTop: '10px' }}>
                                                                <li><strong className="flex-strong">A</strong> = 150 marks</li>
                                                                <li><strong className="flex-strong">B</strong> = 75 marks</li>
                                                                <li><strong className="flex-strong">C</strong> = 40 marks</li>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherAI14Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherAI14Forms[currentPage - 1]._id)}
