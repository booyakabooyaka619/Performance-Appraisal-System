...
export default function ROAI6() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherAI6Forms, setTeacherAI6Forms] = useState([]);
    ...
    const fetchTeacherAI6Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai6form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=AI6`);
            if (!response.ok) throw new Error('Failed to fetch AI6 form');
            const result = await response.json();

            setTeacherAI6Forms(result.reverse());
            ...
            console.error('Error fetching teacher AI6 form:', error);
        }
    };

    useEffect(() => {
        if (teacherAI6Forms.length > 0) {
            const form = teacherAI6Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai6form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/ai6form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#537c78' }}>AI-6 Innovations in TLP</h2>

                {/* Teacher AI6 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherAI6Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherAI6Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherAI6Forms.length === 0 ? (
                        <p className='noprev'>No previous AI6 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherAI6Forms[currentPage - 1]?.numStudentsInnovativeTLP}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>No. of Students Enrollment for the Course:</strong></td>
                                        <td>{teacherAI6Forms[currentPage - 1]?.numStudentsEnrollment}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Quality of Assignments Score:</strong></td>
                                        <td>{teacherAI6Forms[currentPage - 1]?.qualityAssignments}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Quality of Quizzes/Tests Score:</strong></td>
                                        <td>{teacherAI6Forms[currentPage - 1]?.qualityQuizzes}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Quality of Experiment Conducted in Practical Score:</strong></td>
                                        <td>{teacherAI6Forms[currentPage - 1]?.qualityExperiment}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Activities for Engagement with Students Outside the Classroom Score:</strong></td>
                                        <td>{teacherAI6Forms[currentPage - 1]?.engagementActivities}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Activities for Slow Learners Score:</strong></td>
                                        <td>{teacherAI6Forms[currentPage - 1]?.slowLearnerActivities}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Activities for Advance Learners Score:</strong></td>
                                        <td>{teacherAI6Forms[currentPage - 1]?.advanceLearnerActivities}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>No. of Assessment Activities (0-4):</strong></td>
                                        <td>{teacherAI6Forms[currentPage - 1]?.numAssessmentActivities}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Mapping:</strong></td>
                                        <td>{teacherAI6Forms[currentPage - 1]?.mapping}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherAI6Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Evidence Link:</strong></td>
                                        <td>
                                            <a
                                                href={teacherAI6Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherAI6Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherAI6Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherAI6Forms[currentPage - 1]._id)}
