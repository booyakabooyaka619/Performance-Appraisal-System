...
export default function ROPDB2() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherPDB2Forms, setTeacherPDB2Forms] = useState([]);
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/pdb2form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=PDB2`);
    ...
    const fetchTeacherPDB2Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/pdb2form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=PDB2`);
            if (!response.ok) throw new Error('Failed to fetch PDB2 form');
            const result = await response.json();

            setTeacherPDB2Forms(result.reverse());
            ...
            console.error('Error fetching teacher PDB2 form:', error);
        }
    };

    useEffect(() => {
        if (teacherPDB2Forms.length > 0) {
            const form = teacherPDB2Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/pdb2form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/pdb2form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#ffd45b' }}>PDB-2 Service to community through Product Development
                </h2>
                {/* Teacher PDB2 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherPDB2Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherPDB2Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherPDB2Forms.length === 0 ? (
                        <p className='noprev'>No previous PDB2 submissions found for the selected teacher.</p>
                    ) : (
                        <div className="submitted-form">
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        <td><strong>Product Developed For:</strong></td>
                                        <td>{teacherPDB2Forms[currentPage - 1]?.productDevelopedFor}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Training Imparted To:</strong></td>
                                        <td>{teacherPDB2Forms[currentPage - 1]?.trainingImpartedTo}</td>
                                    </tr>

                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherPDB2Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherPDB2Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherPDB2Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    <tr>
                                                ...
                                                <Modal.Body>
                                                    <ul>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            The score is calculated by **adding** the selected values from both categories:
                                                            <strong> Product Developed For</strong> and <strong>Training Imparted To</strong>.
                                                        </li>
                                                        <li>
                                                            After summing up both values, the result is multiplied by 400 to get the final score.
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            <strong>Example:</strong> If the selected values are:<br />
                                                            Product Developed For = 1.5, Training Imparted To = 1.25
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            Then Final Score = <code>(1.5 + 1.25) × 400 = 1,100</code>
                                                        </li>
                                                        <li>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherPDB2Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherPDB2Forms[currentPage - 1]._id)}
