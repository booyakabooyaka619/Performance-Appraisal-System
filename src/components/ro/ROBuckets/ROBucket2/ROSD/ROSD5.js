...
export default function ROSD5() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherSD5Forms, setTeacherSD5Forms] = useState([]);
    ...
    const fetchTeacherSD5Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/sd5form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=SD5`);
            if (!response.ok) throw new Error('Failed to fetch SD5 form');
            const result = await response.json();

            setTeacherSD5Forms(result.reverse());
            ...
            console.error('Error fetching teacher SD5 form:', error);
        }
    };

    useEffect(() => {
        if (teacherSD5Forms.length > 0) {
            const form = teacherSD5Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/sd5form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/sd5form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
            ...
            <div className="bt-container">
                ...
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherSD5Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherSD5Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherSD5Forms.length === 0 ? (
                        <p className='noprev'>No previous SD5 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherSD5Forms[currentPage - 1]?.noOfMeetings}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Awards Won:</strong></td>
                                        <td>{teacherSD5Forms[currentPage - 1]?.awardsWon}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Mentee Development Activities:</strong></td>
                                        <td>{teacherSD5Forms[currentPage - 1]?.menteeDevelopment}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Final Year Feedback:</strong></td>
                                        <td>{teacherSD5Forms[currentPage - 1]?.finalYearFeedback}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherSD5Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a
                                                href={teacherSD5Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherSD5Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherSD5Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherSD5Forms[currentPage - 1]._id)}
