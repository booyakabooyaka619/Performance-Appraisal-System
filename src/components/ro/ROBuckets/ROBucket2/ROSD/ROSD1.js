...
export default function ROSD1() {
    const [submittedForms, setSubmittedForms] = useState([]);
    const [teacherSD1Forms, setTeacherSD1Forms] = useState([]);
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/sd1form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=SD1`);
    ...
    const fetchTeacherSD1Form = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/sd1form/submissions?teacherName=${teacherName}&teacherDepartment=${teacherDepartment}&employeeCode=${employeeCode}&formId=SD1`);
            if (!response.ok) throw new Error('Failed to fetch SD1 form');
            const result = await response.json();

            setTeacherSD1Forms(result.reverse());
            ...
            console.error('Error fetching teacher SD1 form:', error);
        }
    };
    ...
    useEffect(() => {
        if (teacherSD1Forms.length > 0) {
            const form = teacherSD1Forms[currentPage - 1]; // Get the current page form
    ...
    const editReview = async (formId) => {
        try {
            const response = await fetch(`http://localhost:5000/api/sd1form/toggle-edit-review/${formId}`, {
                method: 'PUT',
                headers: {
    ...
    const submitReview = async (formId) => {
        ...
        try {
            ...
            const response = await fetch(`http://localhost:5000/api/sd1form/submit-review/${formId}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#7ba591' }}>SD-1 Average student attendance (during PA evaluation period)
                </h2>

                {/* Teacher SD1 Form Section */}
                <div className="submitted-forms-container">
                    <h4 style={{ textAlign: 'center', backgroundColor: '#2e4756', padding: '10px', borderRadius: '5px', color: 'white', fontSize: '18px', fontWeight: 'bold' }}>
                        {teacherSD1Forms.length > 0 ? `Reviewing submissions for ${teacherName} - Page ${currentPage} of ${teacherSD1Forms.length}`
                            : `Submission 0 of 0`}
                    </h4>

                    {teacherSD1Forms.length === 0 ? (
                        <p className='noprev'>No previous SD1 submissions found for the selected teacher.</p>
                    ) : (
                            ...
                            <table className="submitted-table" style={{ marginTop: '0' }}>
                                <tbody>
                                    <tr>
                                        ...
                                        <td>{teacherSD1Forms[currentPage - 1]?.courseName}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Course Attendance (%):</strong></td>
                                        <td>{teacherSD1Forms[currentPage - 1]?.courseAttendance}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Self Score:</strong></td>
                                        <td>{teacherSD1Forms[currentPage - 1]?.score}</td>
                                    </tr>
                                    <tr>
                                        <td><strong>Evidence Link:</strong></td>
                                        <td>
                                            <a
                                                href={teacherSD1Forms[currentPage - 1]?.linkEvidence}
                                                target="_blank"
                                                rel="noopener noreferrer"
                                            >
                                                {teacherSD1Forms[currentPage - 1]?.linkEvidence}
                                            </a>
                                        </td>
                                    </tr>
                                                ...
                                                <Modal.Body>
                                                    <ul>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            The score is calculated based on the percentage of attendance in the course.
                                                        </li>
                                                        <li>
                                                            Depending on the attendance, the following score thresholds are applied:
                                                            <ul>
                                                                <li>If the attendance is 80% or more, the score is 300.</li>
                                                                <li>If the attendance is between 70% and 79%, the score is 225.</li>
                                                                <li>If the attendance is between 60% and 69%, the score is 150.</li>
                                                                <li>If the attendance is between 50% and 59%, the score is 105.</li>
                                                                <li>If the attendance is between 40% and 49%, the score is 70.</li>
                                                                <li>If the attendance is less than 40%, the score is 0.</li>
                                                            </ul>
                                                        </li>
                                                        <li style={{ marginBottom: '20px' }}>
                                                            <strong>Example:</strong> If the attendance is 75%, then the score will be 225.
                                                        </li>
                                                        <li>
                                                            Higher attendance percentages result in higher scores.
                                                        </li>
                                                    </ul>
                                                </Modal.Body>
                                                <Modal.Footer>
                                    ...
                                    <tr>
                                        <td colSpan="2">
                                            {(!scoreByRO && !reviewByRO) ? (
                                                <button className="submit-review-btn" onClick={() => submitReview(teacherSD1Forms[currentPage - 1]._id)}>
                                                    Submit Review
                                                </button>
                                            ) : (
                                                <>
                                                    {isReviewEditable ? (
                                                        ...
                                                        <Button
                                                            onClick={() => editReview(teacherSD1Forms[currentPage - 1]._id)}
