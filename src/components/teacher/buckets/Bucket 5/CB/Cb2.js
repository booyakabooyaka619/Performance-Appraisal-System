...
export default function Cb9() {
    ...
    useEffect(() => {
        const calculateScore = () => {
            ...
            const selectedScores = [
                scoreMap.expertAICTE[formData.expertAICTE] || 0,
                scoreMap.expertDTE[formData.expertDTE] || 0,
                scoreMap.academicCouncil[formData.academicCouncil] || 0
            ];

            const total = selectedScores.reduce((acc, val) => acc + val, 0);
            setScore(total);
        };
    ...
    const handleConfirmSubmit = (e) => {
        e.preventDefault();
        if (!formData.expertAICTE || !formData.expertDTE || !formData.academicCouncil || !formData.linkEvidence) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/cb2form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/cb2form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleEdit = (form) => {
        setFormData({
            expertAICTE: form.expertAICTE,
            expertDTE: form.expertDTE,
            academicCouncil: form.academicCouncil,
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/cb2form/${deleteFormId}`, { method: 'DELETE' });
                ...
                {!showForm ? (
                                    ...
                                    <table className="submitted-table">
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>{submittedForms[currentPage - 1]?.expertAICTE}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Expert for DTE / University etc:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.expertDTE}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Academic Council / BOS / IQAC Committee:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.academicCouncil}</td>
                                            </tr>
                    ...
                    <form className='form-container' onSubmit={handleConfirmSubmit}>
                        ...
                        <div className="marks-container">
                            ...
                            <div className="marks-item marks-item-value">120</div>
                        ...
                        <h5 className='exp-heading'>Expert Level</h5>

                        <div className='form-group'>
                            <label className='label1'>Expert for AICTE / NBA/NAAC /UGC:
                                <select name="expertAICTE" className="select-input" value={formData.expertAICTE} onChange={handleChange}>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Expert for DTE / University etc:
                                <select name="expertDTE" className="select-input" value={formData.expertDTE} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Yes (40)">Yes (40)</option>
                                    <option value="No (0)">No (0)</option>
                                </select>
                            </label>
                        </div>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Academic council / BOS / IQAC committee:
                                <select name="academicCouncil" className="select-input" value={formData.academicCouncil} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Yes (30)">Yes (30)</option>
                                    <option value="No (0)">No (0)</option>
                                </select>
                            </label>
                        </div>
