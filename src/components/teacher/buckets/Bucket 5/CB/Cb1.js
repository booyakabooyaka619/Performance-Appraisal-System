...
export default function Cb1() {
    ...
    useEffect(() => {
        const calculateScore = () => {
            const scoreMap = {
                ...
                consultancyPosition: {
                    "Individual Consultancy (1.5)": 1.5,
                    "Convenor / PI (1)": 1,
            ...
            const revenueScore = scoreMap.revenueGenerated[formData.revenueGenerated] || 0;
            const positionScore = scoreMap.consultancyPosition[formData.consultancyPosition] || 0;

            const finalScore = Math.round(revenueScore * positionScore * 375);
            setScore(finalScore);
        };
    ...
    const handleConfirmSubmit = (e) => {
        e.preventDefault();
        if (!formData.revenueGenerated || !formData.consultancyPosition || !formData.linkEvidence) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/cb1form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/cb1form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleEdit = (form) => {
        setFormData({
            ...
            consultancyPosition: form.consultancyPosition,
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/cb1form/${deleteFormId}`, { method: 'DELETE' });
                ...
                {!showForm ? (
                                    ...
                                    <table className="submitted-table">
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>{submittedForms[currentPage - 1]?.revenueGenerated}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Consultancy Position:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.consultancyPosition}</td>
                                            </tr>
                    ...
                    <form className='form-container' onSubmit={handleConfirmSubmit}>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Amount of Revenue Generated:
                                <select
                        ...
                        <div className='form-group'>
                            <label className='label1'>Consultancy Position:
                                <select
                                    name="consultancyPosition"
                                    className="select-input"
                                    value={formData.consultancyPosition}
                                    onChange={handleChange}
                                >
                                    <option value="">Select</option>
                                    <option value="Individual Consultancy (1.5)">Individual Consultancy (1.5)</option>
