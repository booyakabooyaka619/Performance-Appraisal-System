...
export default function Sd2() {
    ...
    useEffect(() => {
        calculateScore();
    }, [formData.courseResult]);


    const calculateScore = () => {
        let result = parseFloat(formData.courseResult);
        let score = 0;

        if (result === 100) score = 700;
        else if (result >= 90) score = 590;
        else if (result >= 80) score = 470;
        else if (result >= 70) score = 350;
        else if (result >= 60) score = 240;
        else score = 0;
    ...
    const handleConfirmSubmit = (e) => {
        e.preventDefault();
        if (!formData.courseName || !formData.courseResult || !formData.linkEvidence) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/sd2form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/sd2form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleEdit = (form) => {
        setFormData({
            ...
            courseResult: form.courseResult,
    ...
    const handleDelete = async () => {
        try {
            // Delete the form
            const response = await fetch(`http://localhost:5000/api/sd2form/${deleteFormId}`, { method: 'DELETE' });
                ...
                {!showForm ? (
                                    ...
                                    <table className="submitted-table">
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>{submittedForms[currentPage - 1]?.courseResult}</td>
                                            </tr>
                                            <tr>
                    ...
                    <form className='form-container' onSubmit={handleConfirmSubmit}>
                        ...
                        <div className="marks-container">
                            ...
                            <div className="marks-item marks-item-value">700</div>
                        ...
                        <div className="form-group">
                            <label className='label1'>Course Result (%)
                                <input
                                    type="number"
                                    className='linkinput'
                                    name="courseResult"
                                    value={formData.courseResult}
                                    onChange={handleChange}
                                    min="0"
                                    max="100"
                                />
