...
export default function Sd1() {
    ...
    useEffect(() => {
        calculateScore();
    }, [formData.courseAttendance]);


    const calculateScore = () => {
        let attendance = parseFloat(formData.courseAttendance);
        let score = 0;

        if (attendance >= 80) score = 300;
        else if (attendance >= 70) score = 225;
        else if (attendance >= 60) score = 150;
        else if (attendance >= 50) score = 105;
        else if (attendance >= 40) score = 70;
        else score = 0;
    ...
    const handleConfirmSubmit = (e) => {
        e.preventDefault();
        if (!formData.courseName || !formData.courseAttendance || !formData.linkEvidence) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/sd1form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/sd1form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleEdit = (form) => {
        setFormData({
            ...
            courseAttendance: form.courseAttendance,
    ...
    const handleDelete = async () => {
        try {
            // Delete the form
            const response = await fetch(`http://localhost:5000/api/sd1form/${deleteFormId}`, { method: 'DELETE' });
                                    ...
                                    <table className="submitted-table">
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>{submittedForms[currentPage - 1]?.courseAttendance}</td>
                                            </tr>
                                            <tr>
