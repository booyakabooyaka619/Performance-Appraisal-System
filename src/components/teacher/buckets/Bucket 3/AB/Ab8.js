...
export default function Ab8() {
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/ab8form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=AB8`);
    ...
    useEffect(() => {
        const calculateScore = () => {
            const scoreMap = {
                typeOfMembership: {
                    "Life Membership (first time - 1)": 1,
                    "Life Membership (subsequent - 0.9)": 0.9,
                    "Renewal of yearly Membership (0.8)": 0.8
                },
                positionHeld: {
                    "President / Chair (1.5)": 1.5,
                    "Vice President / Co-chair (1.4)": 1.4,
                    "Executive Secretary (1.3)": 1.3,
                    "Secretary (1.2)": 1.2,
                    "Member Chair (1.1)": 1.1
                }
            };

            const score1 = scoreMap.typeOfMembership[formData.typeOfMembership] || 0;
            const score2 = scoreMap.positionHeld[formData.positionHeld] || 0;
            const finalScore = Math.round(score1 * score2 * 50); // base score is 75
            setScore(finalScore);
        };
    ...
    const handleConfirmSubmit = (e) => {
        e.preventDefault();
        if (!formData.typeOfMembership || !formData.positionHeld || !formData.linkEvidence) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/ab8form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/ab8form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleEdit = (form) => {
        setFormData({
            typeOfMembership: form.typeOfMembership,
            positionHeld: form.positionHeld,
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ab8form/${deleteFormId}`, { method: 'DELETE' });
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#cc222b' }}>AB-8 Association or Membership of Professional Bodies
                </h2>
                ...
                {!showForm ? (
                                        ...
                                        <Modal.Body>
                                            <ul>
                                                <li style={{ marginBottom: '20px' }}>
                                                    ...
                                                    <strong> Type of Membership</strong> and <strong>Position Held in Professional Body</strong>.
                                                </li>
                                                <li>
                                                    After multiplying both the values, the result is multiplied by 50 to get the final score.
                                                </li>
                                                <li style={{ marginBottom: '20px' }}>
                                                    <strong>Example:</strong> If the selected values are:<br />
                                                    Type of Membership = <code>Life Membership (first time) = 1</code><br />
                                                    Position Held = <code>Vice President / Co-chair = 1.4</code>
                                                </li>
                                                <li style={{ marginBottom: '20px' }}>
                                                    Then Final Score = <code>1 × 1.4 × 50 = 70</code>
                                                </li>
                                                <li>
                                                    Higher selections in both categories lead to higher scores.
                                                </li>
                                            </ul>
                                        </Modal.Body>
                                        <Modal.Footer>
                                    ...
                                    <table className="submitted-table">
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>{submittedForms[currentPage - 1]?.typeOfMembership}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Position Held:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.positionHeld}</td>
                                            </tr>
                    ...
                    <form className='form-container' onSubmit={handleConfirmSubmit}>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Type of Membership:
                                <select name="typeOfMembership" className="select-input" value={formData.typeOfMembership} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Life Membership (first time - 1)">Life Membership (first time - 1)</option>
                                    <option value="Life Membership (subsequent - 0.9)">Life Membership (subsequent - 0.9)</option>
                                    <option value="Renewal of yearly Membership (0.8)">Renewal of yearly Membership (0.8)</option>
                                </select>
                            </label>
                        </div>

                        <div className='form-group'>
                            <label className='label1'>Position Held in Professional Body:
                                <select name="positionHeld" className="select-input" value={formData.positionHeld} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="President / Chair (1.5)">President / Chair (1.5)</option>
                                    <option value="Vice President / Co-chair (1.4)">Vice President / Co-chair (1.4)</option>
                                    <option value="Executive Secretary (1.3)">Executive Secretary (1.3)</option>
                                    <option value="Secretary (1.2)">Secretary (1.2)</option>
                                    <option value="Member Chair (1.1)">Member Chair (1.1)</option>
                                </select>
                            </label>
                        </div>
                        {/* Evidence Link Input */}
