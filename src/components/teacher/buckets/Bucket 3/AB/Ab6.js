...
export default function Ab6() {
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/ab6form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=AB6`);
    ...
    useEffect(() => {
        const calculateScore = () => {
            const scoreMap = {
                ...
                execution: {
                    "100% activities /events conducted (2)": 2,
                    "75% activities /events conducted (1)": 1,
                    "50% activities /events conducted (0.5)": 0.5
                }
            };

            const selectedScores = [
                scoreMap.designation[formData.designation] || 0,
                scoreMap.certification[formData.certification] || 0,
                scoreMap.execution[formData.execution] || 0
            ];
    ...
    const handleConfirmSubmit = (e) => {
        e.preventDefault();
        if (!formData.designation || !formData.certification || !formData.execution || !formData.linkEvidence) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/ab6form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/ab6form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ab6form/${deleteFormId}`, { method: 'DELETE' });
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#cc222b' }}>AB-6 Institutional Committee Activities
                </h2>
                                ...
                                <div className="submitted-form">
                                        ...
                                        <Modal.Body>
                                            <ul>
                                                <li style={{ marginBottom: '20px' }}>
                                                    The score is calculated by multiplying the values from the following three categories:
                                                    <ul style={{ paddingLeft: '20px' }}>
                                                        <li><strong>Designation</strong> (e.g., Convener or Member)</li>
                                                        <li><strong>Planning Certification</strong> (Certified or Not Certified)</li>
                                                        <li><strong>Execution of Activities/Events</strong> (percentage of events conducted)</li>
                                                    </ul>
                                                </li>
                                                <li>
                                                    The final score is the product of these values, multiplied by <strong>100</strong>.
                                                </li>
                                                <li style={{ marginBottom: '20px' }}>
                                                    <strong>Example:</strong> If the selected values are:<br />
                                                    Designation = 1 (Committee Convener),<br />
                                                    Certification = 1 (Certified),<br />
                                                    Execution = 2 (100% activities conducted)
                                                </li>
                                                <li style={{ marginBottom: '20px' }}>
                                                    Then Final Score = <code>1 × 1 × 2 × 100 = 200</code>
                                                </li>
                                                <li>
                                                    All three selections impact the final score. A low value in any category will reduce the total significantly.
                                                </li>
                                            </ul>
                                        </Modal.Body>
                                        <Modal.Footer>
                                    ...
                                    <table className="submitted-table">
                                        <tbody>
                                            ...
                                            <tr>
                                                <td><strong>Planning Certification:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.certification}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Execution of Activities:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.execution}</td>
                                            </tr>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Designation:
                                <select name="designation" className="select-input" value={formData.designation} onChange={handleChange}>
                                    ...
                                    <option value="Committee Convener (1)">Committee Convener (1)</option>
                                    <option value="Member (0.6)">Member (0.6)</option>
                                </select>
                            </label>
                        </div>

                        <div className='form-group'>
                            <label className='label1'>Planning Certification from RO:
                                <select name="certification" className="select-input" value={formData.certification} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Certified (1)">Certified (1)</option>
                                    <option value="Not Certified (0)">Not Certified (0)</option>
                                </select>
                            </label>
                        </div>

                        <div className='form-group'>
                            <label className='label1'>Execution of Activities / Events Planned:
                                <select name="execution" className="select-input" value={formData.execution} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="100% activities /events conducted (2)">100% activities /events conducted (2)</option>
                                    <option value="75% activities /events conducted (1)">75% activities /events conducted (1)</option>
                                    <option value="50% activities /events conducted (0.5)">50% activities /events conducted (0.5)</option>
                                </select>
                            </label>
                        </div>
                        {/* Evidence Link Input */}
