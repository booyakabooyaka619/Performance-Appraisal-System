...
export default function Ab3() {
    ...
    useEffect(() => {
        const calculateScore = () => {
            ...
            const selectedScores = [
                scoreMap.studentsParticipated[formData.studentsParticipated] || 0,
                scoreMap.activityDuration[formData.activityDuration] || 0,
                scoreMap.feedback[formData.feedback] || 0
            ];

            const product = selectedScores.reduce((acc, score) => acc * score, 1);
            setScore(Math.round(product * 100)); // Final formula
        };
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/ab3form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/ab3form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleEdit = (form) => {
        setFormData({
            studentsParticipated: form.studentsParticipated,
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ab3form/${deleteFormId}`, { method: 'DELETE' });
                ...
                {!showForm ? (
                                    ...
                                    <table className="submitted-table">
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>{submittedForms[currentPage - 1]?.studentsParticipated}</td>
                                            </tr>
                                            <tr>
                    ...
                    <form className='form-container' onSubmit={handleConfirmSubmit}>
                        ...
                        <div className="marks-container">
                            ...
                            <div className="marks-item marks-item-value">219</div>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Number of Students Participated:
                                <select name="studentsParticipated" className="select-input" value={formData.studentsParticipated} onChange={handleChange}>
                                    ...
                                    <option value="Between 10 to 15 (0.8)">Between 10 to 15 (0.8)</option>
                                </select>
                            </label>
                        </div>

                        <div className='form-group'>
                            <label className='label1'>Activity Duration:
                                <select name="activityDuration" className="select-input" value={formData.activityDuration} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Two weeks (1.75)">Two weeks (1.75)</option>
                                    <option value="One week (1.5)">One week (1.5)</option>
                                    <option value="Three days (1)">Three days (1)</option>
                                    <option value="Two days (0.8)">Two days (0.8)</option>
                                    <option value="One Day (0.6)">One Day (0.6)</option>
                                </select>
                            </label>
                        </div>

                        <div className='form-group'>
                            <label className='label1'>Students’ Feedback:
                                <select name="feedback" className="select-input" value={formData.feedback} onChange={handleChange}>
                                    <option value="">Select</option>
