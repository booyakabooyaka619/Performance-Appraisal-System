...
export default function Ab5() {
    ...
    useEffect(() => {
        const calculateScore = () => {
            ...
            const selected = scoreMap[formData.noOfRoles] || 0;
            setScore(selected * 50); // Example: Max = 3 x 25 = 75
        };
    ...
    const handleConfirmSubmit = (e) => {
        e.preventDefault();
        if (!formData.noOfRoles || !formData.roleType || !formData.linkEvidence) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/ab5form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/ab5form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ab5form/${deleteFormId}`, { method: 'DELETE' });
                ...
                {!showForm ? (
                            ...
                            {submittedForms.length === 0 ? (
                                ...
                                <div className="submitted-form">
                                    ...
                                    <table className="submitted-table">
                                        <tbody>
                                            <tr>
                                                <td><strong>Type of Role</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.roleType}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>No. of Roles:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.noOfRoles}</td>
                                            </tr>
                                            <tr>
                    ...
                    <form className='form-container' onSubmit={handleConfirmSubmit}>
                        ...
                        <div className="form-group">
                            <label className='label1'>Type of Role
                                <input
                                    type="text"
                                    className='linkinput'
                                    name="roleType"
                        ...
                        <div className='form-group'>
                            <label className='label1'>No. of Roles:
                                <select name="noOfRoles" className="select-input" value={formData.noOfRoles} onChange={handleChange}>
