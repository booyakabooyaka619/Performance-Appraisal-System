...
export default function Rb5() {
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/rb5form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=RB5`);
    ...
    useEffect(() => {
        const calculateScore = () => {
            const scoreMap = {
                universityQuality: {
                    "IITs/ Foreign reputed Universities /MNC(1.5)": 1.5,
                    "NITs/Non-MNC (1.35)": 1.35,
                    "Autonomous colleges/SME (1.25)": 1.25,
                    "State Universities (1.1)": 1.1,
                    "State Colleges (1.0)": 1,
                    "Unrecognised University/Institute (0)": 0,
                },
                qualificationLevel: {
                    "Ph.D.(1)": 1,
                    "Value addition or certificate courses from industry(0.8)": 0.8,
                    "MOOC Courses e.g., NPTEL (0.6)": 0.6,
                },
                gradeAchieved: {
                    "A+ and above (1.25)": 1.25,
                    "A(1)": 1,
                    "B+(0.8)": 0.8,
                    "Grade B and below (0.3)": 0.3,
                    "Fail (0)": 0,
                }
            };

            const selectedScores = [
                scoreMap.universityQuality[formData.universityQuality] || 0,
                scoreMap.qualificationLevel[formData.qualificationLevel] || 0,
                scoreMap.gradeAchieved[formData.gradeAchieved] || 0,
            ];

            const product = selectedScores.reduce((acc, score) => acc * score, 1);
            setScore(Math.round(product * 25));
        };
    ...
    const handleConfirmSubmit = (e) => {
        e.preventDefault();
        if (!formData.universityQuality || !formData.qualificationLevel || !formData.gradeAchieved || !formData.linkEvidence) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/rb5form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/rb5form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleEdit = (form) => {
        setFormData({
            universityQuality: form.universityQuality,
            qualificationLevel: form.qualificationLevel,
            gradeAchieved: form.gradeAchieved,
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb5form/${deleteFormId}`, { method: 'DELETE' });
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#f15b4c ' }}>RB-5 Qualification Upgradation  (Degree, Cources, Certificate)
                </h2>
                ...
                {!showForm ? (
                            ...
                            {submittedForms.length === 0 ? (
                                ...
                                <div className="submitted-form">
                                        ...
                                        <Modal.Body>
                                            <ul>
                                                <li style={{ marginBottom: '20px' }}>
                                                    ...
                                                    <strong> Quality of the University, Level of Qualification, and Grade Achieved.</strong>
                                                </li>
                                                <li>
                                                    Each selection has a predefined multiplier value. The product of these values is multiplied by 25 to compute the final score.
                                                </li>
                                                <li style={{ marginBottom: '20px' }}>
                                                    <strong>Example:</strong> If the selected values are:<br />
                                                    University Quality = 1.35 (NITs/Non-MNC)<br />
                                                    Qualification Level = 0.8 (Value addition course)<br />
                                                    Grade Achieved = 1 (A)
                                                </li>
                                                <li style={{ marginBottom: '20px' }}>
                                                    Then Final Score = <code>1.35 × 0.8 × 1 × 25 = 27</code>
                                                </li>
                                                <li>
                                                    Selecting higher levels of institutions, qualifications, and grades will result in a higher final score.
                                                </li>
                                            </ul>
                                        </Modal.Body>
                                        <Modal.Footer>
                                    ...
                                    <table className="submitted-table">
                                        <tbody>
                                            <tr>
                                                <td><strong>Quality of the University:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.universityQuality}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Level of Qualification:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.qualificationLevel}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Grades Achieved:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.gradeAchieved}</td>
                                            </tr>
                                            <tr>
                    ...
                    <form className='form-container' onSubmit={handleConfirmSubmit}>
                        ...
                        <div className="marks-container">
                            ...
                            <div className="marks-item marks-item-value">25</div>
                            <div className="marks-item marks-item-header">Maximum marks with bonus</div>
                            <div className="marks-item marks-item-value">47</div>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Quality of the University:
                                <select name="universityQuality" className="select-input" value={formData.universityQuality} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="IITs/ Foreign reputed Universities /MNC(1.5)">IITs/ Foreign reputed Universities /MNC (1.5)</option>
                                    <option value="NITs/Non-MNC (1.35)">NITs/Non-MNC (1.35)</option>
                                    <option value="Autonomous colleges/SME (1.25)">Autonomous colleges/SME (1.25)</option>
                                    <option value="State Universities (1.1)">State Universities (1.1)</option>
                                    <option value="State Colleges (1.0)">State Colleges (1.0)</option>
                                    <option value="Unrecognised University/Institute (0)">Unrecognised University/Institute (0)</option>
                                </select>
                            </label>
                        </div>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Level of Qualification:
                                <select name="qualificationLevel" className="select-input" value={formData.qualificationLevel} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Ph.D.(1)">Ph.D. (1)</option>
                                    <option value="Value addition or certificate courses from industry(0.8)">Value Addition/Industry Certificate (0.8)</option>
                                    <option value="MOOC Courses e.g., NPTEL (0.6)">MOOC Courses e.g., NPTEL (0.6)</option>
                                </select>
                            </label>
                        </div>
                        ...
                        {/* Grades */}
                        <div className='form-group'>
                            <label className='label1'>Grades:
                                <select name="gradeAchieved" className="select-input" value={formData.gradeAchieved} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="A+ and above (1.25)">A+ and above (1.25)</option>
                                    <option value="A(1)">A (1)</option>
                                    <option value="B+(0.8)">B+ (0.8)</option>
                                    <option value="Grade B and below (0.3)">Grade B and below (0.3)</option>
                                    <option value="Fail (0)">Fail (0)</option>
                                </select>
                            </label>
                        </div>
