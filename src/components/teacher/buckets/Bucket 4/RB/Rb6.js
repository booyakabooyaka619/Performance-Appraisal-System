...
export default function Rb6() {
    ...
    useEffect(() => {
        const calculateScore = () => {
            ...
            const selectedScore = scoreMap[formData.awardType] || 0;
            setScore(Math.round(selectedScore * 50));
        };
    ...
    const handleConfirmSubmit = (e) => {
        e.preventDefault();
        if (!formData.awardType || !formData.linkEvidence) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/rb6form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/rb6form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb6form/${deleteFormId}`, { method: 'DELETE' });
                ...
                {!showForm ? (
                                    ...
                                    <table className="submitted-table">
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>{submittedForms[currentPage - 1]?.awardType}</td>
                                            </tr>
                                            <tr>
                    ...
                    <form className='form-container' onSubmit={handleConfirmSubmit}>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Award Type:
                                <select name="awardType" className="select-input" value={formData.awardType} onChange={handleChange}>
