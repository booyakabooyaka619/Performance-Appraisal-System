...
export default function Rb10() {
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/rb10form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=RB10`);
    ...
    useEffect(() => {
        const calculateScore = () => {
            const interactionScores = {
                "Pursuing Externally funded Project / research project / Consultancy / Board Member  of Pvt./Public Ltd  Company / Technical Know-how Transfer (including Pro-bono) to Industry (1.75)": 1.75,
                "MOU (1.5)": 1.5,
                "Research Paper (1.25)": 1.25,
                "Book (1.25)": 1.25,
                "Development of semester- long course / teaching modules(1)": 1,
                "Technical Article (0.8)": 0.8,
                "Community services(0.8)": 0.8,
                "Expert Talk (minimum 3) (0.7)": 0.7,
                "Training to students / VAC (0.4)": 0.4
            };

            const partnerScores = {
                "Institute - International (1.25)": 1.25,
                "Institute - National Importance (1)": 1,
                "Institute - State Importance (0.9)": 0.9,
                "Institute - State College (0.8)": 0.8,
                "Industry - MNC (1.25)": 1.25,
                "Industry - Non-MNC (1)": 1,
                "Industry - SME (0.8)": 0.8,
            };

            const score1 = interactionScores[formData.typeOfInteraction] || 0;
            const score2 = partnerScores[formData.typeOfPartner] || 0;

            const finalScore = Math.round(score1 * score2 * 50);
            setScore(finalScore);
    ...
    const handleConfirmSubmit = (e) => {
        e.preventDefault();
        if (!formData.typeOfInteraction || !formData.typeOfPartner || !formData.linkEvidence) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/rb10form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/rb10form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleEdit = (form) => {
        setFormData({
            typeOfInteraction: form.typeOfInteraction,
            typeOfPartner: form.typeOfPartner,
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb10form/${deleteFormId}`, { method: 'DELETE' });
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#f15b4c' }}>RB-10 Interaction with outside
                </h2>
                ...
                {!showForm ? (
                            ...
                            {submittedForms.length === 0 ? (
                                ...
                                <div className="submitted-form">
                                        ...
                                        <Modal.Body>
                                            <ul>
                                                <li style={{ marginBottom: '20px' }}>
                                                    The score is calculated by multiplying the selected values from two categories:
                                                    <strong> Types of Interaction</strong> and <strong>Type of Partner</strong>.
                                                </li>
                                                <li>
                                                    After multiplying the two values, the result is multiplied by <strong>50</strong> to get the final score.
                                                </li>
                                                <li style={{ marginBottom: '20px' }}>
                                                    <strong>Example:</strong> If the selected values are:<br />
                                                    Types of Interaction = 1.75 (e.g., Externally funded Project)<br />
                                                    Type of Partner = 1.25 (e.g., Industry - MNC)
                                                </li>
                                                <li style={{ marginBottom: '20px' }}>
                                                    Then Final Score = <code>1.75 × 1.25 × 50 = 109.38</code>
                                                </li>
                                                <li>
                                                    Selecting higher-valued options from both categories will result in a higher score.
                                                </li>
                                            </ul>
                                        </Modal.Body>
                                        <Modal.Footer>
                                    ...
                                    <table className="submitted-table">
                                        <tbody>
                                            <tr>
                                                <td><strong>Types of Interaction:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.typeOfInteraction}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Type of Partner:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.typeOfPartner}</td>
                                            </tr>
                    ...
                    <form className='form-container' onSubmit={handleConfirmSubmit}>
                        ...
                        <div className="marks-container">
                            ...
                            <div className="marks-item marks-item-value">109</div>
                        ...
                        {/* Types of Interaction Dropdown */}
                        <div className='form-group'>
                            <label className='label1'>Types of Interaction:
                                <select name="typeOfInteraction" className="select-input" value={formData.typeOfInteraction} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Pursuing Externally funded Project / research project / Consultancy / Board Member  of Pvt./Public Ltd  Company / Technical Know-how Transfer (including Pro-bono) to Industry (1.75)">
                                        Pursuing Externally funded Project / research project / Consultancy / Board Member of Pvt./Public Ltd Company / Technical Know-how Transfer (1.75)
                                    </option>
                                    <option value="MOU (1.5)">MOU (1.5)</option>
                                    <option value="Research Paper (1.25)">Research Paper (1.25)</option>
                                    <option value="Book (1.25)">Book (1.25)</option>
                                    <option value="Development of semester- long course / teaching modules(1)">Development of semester-long course / teaching modules (1)</option>
                                    <option value="Technical Article (0.8)">Technical Article (0.8)</option>
                                    <option value="Community services(0.8)">Community services (0.8)</option>
                                    <option value="Expert Talk (minimum 3) (0.7)">Expert Talk (minimum 3) (0.7)</option>
                                    <option value="Training to students / VAC (0.4)">Training to students / VAC (0.4)</option>
                                </select>
                            </label>
                        </div>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Type of Partner (Institute or Industry):
                                <select name="typeOfPartner" className="select-input" value={formData.typeOfPartner} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Institute - International (1.25)">Institute - International (1.25)</option>
                                    <option value="Institute - National Importance (1)">Institute - National Importance (1)</option>
                                    <option value="Institute - State Importance (0.9)">Institute - State Importance (0.9)</option>
                                    <option value="Institute - State College (0.8)">Institute - State College (0.8)</option>
                                    <option value="Industry - MNC (1.25)">Industry - MNC (1.25)</option>
                                    <option value="Industry - Non-MNC (1)">Industry - Non-MNC (1)</option>
                                    <option value="Industry - SME (0.8)">Industry - SME (0.8)</option>
                                </select>
                            </label>
                        </div>
