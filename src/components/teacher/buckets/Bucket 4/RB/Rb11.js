...
export default function Rb11() {
    ...
    useEffect(() => {
        const calculateScore = () => {
            const scoreMap = {
                qualityOfSpeakers: {
                    "Industry Expert (1.5)": 1.5,
                    "Academia (International) (1.5)": 1.5,
                    "Academia (National) (1.25)": 1.25,
                    "Academia (State) (1)": 1,
                },
                organisingAgency: {
                    "Industry (1.75)": 1.75,
            ...
            const selectedScores = [
                scoreMap.qualityOfSpeakers[formData.qualityOfSpeakers] || 0,
            ...
            setScore(Math.round(product * 50)); // Max marks: 254, base: 75
        };
    ...
    const handleConfirmSubmit = (e) => {
        e.preventDefault();
        if (!formData.qualityOfSpeakers || !formData.organisingAgency || !formData.eventLevel || !formData.linkEvidence) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/rb11form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/rb11form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleEdit = (form) => {
        setFormData({
            qualityOfSpeakers: form.qualityOfSpeakers,
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb11form/${deleteFormId}`, { method: 'DELETE' });
                ...
                {!showForm ? (
                                    ...
                                    <table className="submitted-table">
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>{submittedForms[currentPage - 1]?.qualityOfSpeakers}</td>
                                            </tr>
                                            <tr>
                                            ...
                                            <tr>
                                                ...
                                                <td>{submittedForms[currentPage - 1]?.eventLevel}</td>
                                            </tr>
                    ...
                    <form className='form-container' onSubmit={handleConfirmSubmit}>
                        ...
                        <div className="marks-container">
                            ...
                            <div className="marks-item marks-item-value">164</div>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Quality of Speakers:
                                <select name="qualityOfSpeakers" className="select-input" value={formData.qualityOfSpeakers} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Industry Expert (1.5)">Industry Expert (1.5)</option>
                                    <option value="Academia (International) (1.5)">Academia (International) (1.5)</option>
                                    <option value="Academia (National) (1.25)">Academia (National) (1.25)</option>
                                    <option value="Academia (State) (1)">Academia (State) (1)</option>
                                </select>
                            </label>
                        </div>

                        {/* Dropdown 2: Organising Agency */}
                        <div className='form-group'>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Level:
                                <select name="eventLevel" className="select-input" value={formData.eventLevel} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="International (1.25)">International (1.25)</option>
                                    <option value="National (1.0)">National (1.0)</option>
