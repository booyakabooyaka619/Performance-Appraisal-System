...
export default function Rb9() {
    ...
    const fetchSubmittedForms = async () => {
        if (!user) return;
        try {
            const response = await fetch(`http://localhost:5000/api/rb9form/submissions?teacherName=${user.name}&teacherDepartment=${user.department}&formId=RB9`);
    ...
    useEffect(() => {
        ...
        const selectedScores = [
            scoreMap[formData.technicalContent] || 0,
            scoreMap[formData.presentationSkills] || 0,
            scoreMap[formData.newTopic] || 0,
            scoreMap[formData.literatureSurvey] || 0,
            scoreMap[formData.researchAspects] || 0,
            scoreMap[formData.audienceInteraction] || 0,
        ];

        const total = selectedScores.reduce((sum, val) => sum + val, 0);
        setScore(total);
    }, [formData]);
    ...
    const handleConfirmSubmit = (e) => {
        ...
        const requiredFields = [
            'technicalContent', 'presentationSkills', 'newTopic',
            'literatureSurvey', 'researchAspects', 'audienceInteraction', 'linkEvidence'
        ];
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/rb9form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/rb9form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleEdit = (form) => {
        setFormData({
            ...
            presentationSkills: form.presentationSkills,
            newTopic: form.newTopic,
            literatureSurvey: form.literatureSurvey,
            researchAspects: form.researchAspects,
            audienceInteraction: form.audienceInteraction,
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/rb9form/${deleteFormId}`, { method: 'DELETE' });
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#f15b4c' }}>RB-9 Technical / Non Technical Presentation
                </h2>
                ...
                {!showForm ? (
                            ...
                            {submittedForms.length === 0 ? (
                                ...
                                <div className="submitted-form">
                                        ...
                                        <Modal.Body>
                                            <ul>
                                                <li style={{ marginBottom: '20px' }}>
                                                    The score is calculated by adding the values of the following criteria:
                                                    <br />
                                                    <strong>Technical Content</strong>, <strong>Presentation Skills</strong>, <strong>New Topic</strong>, <strong>Literature Survey</strong>, <strong>Research Aspects</strong>, and <strong>Audience Interaction</strong>.
                                                </li>
                                                <li>
                                                    Each selected option has a score:
                                                    <ul style={{ marginTop: '10px', marginBottom: '10px' }}>
                                                        <li><code>Yes (05)</code> = 5</li>
                                                        <li><code>Yes (04)</code> = 4</li>
                                                        <li><code>No (0)</code> = 0</li>
                                                    </ul>
                                                </li>
                                                <li style={{ marginBottom: '20px' }}>
                                                    <strong>Example:</strong> If the selected values are:<br />
                                                    Technical Content = Yes (05),<br />
                                                    Presentation Skills = Yes (04),<br />
                                                    New Topic = Yes (04),<br />
                                                    Literature Survey = Yes (04),<br />
                                                    Research Aspects = Yes (04),<br />
                                                    Audience Interaction = Yes (04)
                                                </li>
                                                <li style={{ marginBottom: '20px' }}>
                                                    Then Final Score = <code>5 + 4 + 4 + 4 + 4 + 4 = 25</code>
                                                </li>
                                                <li>
                                                    Selecting more "Yes" options with higher scores results in a higher total score.
                                                </li>
                                            </ul>
                                        </Modal.Body>
                                        <Modal.Footer>
                                    ...
                                    <table className="submitted-table">
                                        <tbody>
                                            <tr>
                                                <td><strong>Technical / Non Technical Content:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.technicalContent}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Presentation Skills:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.presentationSkills}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>New/emerging/cutting edge Topic:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.newTopic}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Literature Survey:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.literatureSurvey}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Research Aspects:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.researchAspects}</td>
                                            </tr>
                                            <tr>
                                                <td><strong>Audience Interaction:</strong></td>
                                                <td>{submittedForms[currentPage - 1]?.audienceInteraction}</td>
                                            </tr>
                    ...
                    <form className='form-container' onSubmit={handleConfirmSubmit}>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Technical / Non Technical Content:
                                <select name="technicalContent" className="select-input" value={formData.technicalContent} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Yes (05)">Yes (05)</option>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Presentation Skills:
                                <select name="presentationSkills" className="select-input" value={formData.presentationSkills} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Yes (04)">Yes (04)</option>
                                    <option value="No (0)">No (0)</option>
                                </select>
                            </label>
                        </div>
                        ...
                        <div className='form-group'>
                            <label className='label1'>New/emerging/cutting edge Topic:
                                <select name="newTopic" className="select-input" value={formData.newTopic} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Yes (04)">Yes (04)</option>
                                    <option value="No (0)">No (0)</option>
                                </select>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Literature Survey:
                                <select name="literatureSurvey" className="select-input" value={formData.literatureSurvey} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Yes (04)">Yes (04)</option>
                                    <option value="No (0)">No (0)</option>
                                </select>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Research Aspects:
                                <select name="researchAspects" className="select-input" value={formData.researchAspects} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Yes (04)">Yes (04)</option>
                                    <option value="No (0)">No (0)</option>
                                </select>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Audience Interaction:
                                <select name="audienceInteraction" className="select-input" value={formData.audienceInteraction} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Yes (04)">Yes (04)</option>
                                    <option value="No (0)">No (0)</option>
                                </select>
