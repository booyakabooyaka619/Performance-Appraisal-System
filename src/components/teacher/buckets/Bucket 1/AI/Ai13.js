...
export default function Ai13() {
    ...
    // calculateScore function to calculate the score based on the previewGrade
    const calculateScore = (data) => {
        ...
        return gradeScores[data.previewGrade] || 0; // Default to 0 if no previewGrade is selected
    };

    // useEffect hook to update the score when formData changes
    useEffect(() => {
    ...
    const handleConfirmSubmit = (e) => {
        e.preventDefault();
        if (!formData.previewGrade || !formData.linkEvidence) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/ai13form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/ai13form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleEdit = (form) => {
        setFormData({
            previewGrade: form.previewGrade,
    ...
    const handleDelete = async () => {
        try {
            // Delete the form
            const response = await fetch(`http://localhost:5000/api/ai13form/${deleteFormId}`, { method: 'DELETE' });
    ...
    return (
        <>
            ...
            <div className="bt-container">
                <h2 style={{ backgroundColor: '#537c78' }}>AI-13  Preview Grade (preceding semester of PA evaluation)</h2>
                ...
                {!showForm ? (
                                    ...
                                    <table className="submitted-table">
                                        <tbody>
                                            <tr>
                                                ...
                                                <td>{submittedForms[currentPage - 1]?.previewGrade}</td>
                                            </tr>
                                            <tr>
                    ...
                    <form className='form-container' onSubmit={handleConfirmSubmit}>
                        ...
                        <div className="form-group">
                            <label className='label1'>Preview Grade
                                <select
                                    className='input-field'
                                    name="previewGrade"
                                    value={formData.previewGrade}
