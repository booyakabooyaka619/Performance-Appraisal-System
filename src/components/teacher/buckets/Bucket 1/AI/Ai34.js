...
export default function Ai34() {
    ...
    useEffect(() => {
        const calculateScore = () => {
            ...
            setScore(Math.round(product * 75));
        };

        calculateScore();
    }, [formData]); // Only re-run when formData changes

    const handleConfirmSubmit = (e) => {
        e.preventDefault();
        if (!formData.typeOfGuide || !formData.typeOfOrganisation || !formData.typeOfProject || !formData.mapping || !formData.linkEvidence) {
    ...
    const handleSubmit = async () => {
        try {
            ...
            if (isEditing) {
                response = await fetch(`http://localhost:5000/api/ai34form/${editFormId}`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
            } else {
                response = await fetch('http://localhost:5000/api/ai34form', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(dataToSubmit),
                });
    ...
    const handleDelete = async () => {
        try {
            const response = await fetch(`http://localhost:5000/api/ai34form/${deleteFormId}`, { method: 'DELETE' });
                ...
                {!showForm ? (
                    ...
                    <form className='form-container' onSubmit={handleConfirmSubmit}>
                        ...
                        <div className="marks-container">
                            ...
                            <div className="marks-item marks-item-value">254</div>
                        ...
                        {/* Type of Organisation of Industry Co-guide Dropdown */}
                        <div className='form-group'>
                        ...
                        <div className='form-group'>
                            <label className='label1'>Type of Project:
                                <select name="typeOfProject" className="select-input" value={formData.typeOfProject} onChange={handleChange}>
                                    <option value="">Select</option>
                                    <option value="Functional Project (1)">Functional Project (1)</option>
